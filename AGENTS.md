...
- Do not add URL-shaped product endpoints unless there is a strong compatibility reason. The current explicit exception is the standard MCP Streamable HTTP endpoint `POST /_p2p/mcp`; other product capabilities should still prefer stable product actions and documented `params` schemas.
