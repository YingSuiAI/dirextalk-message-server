...
- Agent/API: Agent config/password are owner-token operations. Agent config includes display fields such as `display_name`/`avatar_url` and the durable MCP room blacklist `mcp_blocked_room_ids`; MCP actions and the standard MCP endpoint must not use blacklisted rooms, filtering them from room search and rejecting direct access with 403. Agent tokens may call only `agent.matrix_session.create`, temporary fixed MCP HTTP action wrappers, and `POST /mcp`; they cannot create realtime WS tickets. `POST /mcp` is the standard MCP Streamable HTTP endpoint for external MCP clients and supports JSON-RPC `initialize`, `tools/list`, and `tools/call` over POST with `Authorization: Bearer <agent_token>` only; it rejects bearer tokens in query strings, validates `Origin`, returns 405 for GET/SSE while server-to-client streaming is unused, and must never pass the inbound MCP bearer token to downstream services. MCP read actions use RFC3339/RFC3339Nano `from_time`/`to_time`, opaque stable snapshot `cursor`, and readable response fields such as `created_at`, `last_message_at`, and string `joined_at`; do not return or document old MCP `ts`/`last_ts` fields. Channel posts/comments and ordinary channel chat stay separate: `mcp.channel_posts.list` returns post summaries with comment/like/local-favorite counts, `mcp.channel_comments.list` returns comment details, and channel chat uses `mcp.messages.list`. `agent.matrix_session.create` returns a Matrix Client-Server session for the local `@agent:<server>` bridge user and must not evict the owner's devices. Owner clients must read bridge online state from native Matrix room state in the real `agent_room_id`: `io.dirextalk.agent.status` with state key `@agent:<server>` and content field `online`. The running local Agent bridge publishes `online=true/false` through its Matrix `@agent:<server>` session; the server must not infer online from `agent.config.enabled`, `/sync`, or WS sessions. Server startup/repair and `agent.config.update enabled=false` may publish `online=false` as a safe fallback. `sync.bootstrap` only returns `agent_room_id`; do not add `agent_online` back to bootstrap and do not emit `agent.presence` events. `agent.status`/`agents.status` are removed and must not be used. The real `agent_room_id` defaults to no system push for the portal owner through a room-level Matrix push rule with empty actions; preserve an existing explicit rule for that room. Native Agent in the message-server owns standard MCP client wiring, skills, model provider request handling, Eino orchestration, runtime CLI tools, and built-in Dirextalk tools; the backend still keeps temporary fixed `mcp.*` capability wrappers and owner-only `plugins.*` management actions for non-Agent plugins. Fixed `mcp.*` body actions, Native Agent built-in Dirextalk tools, and `POST /mcp` must share the same `internal/dirextalkmcp` registry, schemas, pagination helpers, room authorization, DTOs, and invocation service; `p2p` adapts store/transport/history/profile/blocklist dependencies and must not fork MCP business logic inside `nativeagent`, the temporary wrappers, or the HTTP transport. The next MCP-D pass should delete fixed `mcp.*` body actions unless product explicitly extends compatibility. Native Agent runtime config is stored in native portal Agent config storage, and old hidden `io.dirextalk.agent` plugin config is only a sanitized startup migration source. Model provider API keys are request-scoped Native Agent inputs: clients may pass the selected `model_profile` with `api_key` only on direct `agent.*` calls or `client.native_agent_stream` frames, and the message-server must not persist, return, or inject those keys into plugin or runtime env.
