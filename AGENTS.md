...
Protected product actions require `Authorization: Bearer <access_token>` when issued through retained HTTP routes. Logged-in client product actions use `GET /_p2p/ws` `client.request` frames after creating a `realtime.ws_ticket.create` ticket; ordinary timeline/media/history/search/redaction still use Matrix Client-Server APIs. HTTP `/query` and `/command` remain for health/bootstrap/auth/status/password, WS ticket creation, fixed MCP actions, and node-to-node public/callback actions. `agent_token` is accepted only for fixed `mcp.*` HTTP actions and `realtime.ws_ticket.create`, so agent gateways can connect WS, passively receive `agent_room.message`, and send ephemeral `client.agent_stream` fragments. MCP actions must not be migrated into WS `client.request`. `GET /_p2p/ws` authenticates only a short-lived single-use WS ticket, not a bearer token. Public actions are `portal.bootstrap`, `portal.auth`, `portal.status`, `contacts.reactivate`, `channels.public.search`, `channels.public.get`, `channels.public.join_request`, `channels.public.join_result`, and `users.public_channels`. `channels.public.join_result` is an internal node-to-node approval callback, not a normal client workflow entry.
...
- The configured agents room is the narrow gateway exception: backend startup must create and persist a real private Matrix room id for `agent_room_id`, join both owner and local `@agent:<server>` to that room, and ordinary messages in that room may emit `agent_room.message` gateway events for local agent gateways. Gateway replies must be sent by `@agent:<server>`, not owner. Agent WS stream fragments are ephemeral owner-display hints only; final recoverable replies must still be Matrix `m.room.message` events. Do not use legacy pseudo ids such as `!agent:<domain>`.
...
- Agent/API: Agent config/password are owner-token operations. Agent tokens may call only MCP actions and create realtime WS tickets for passive gateway listening. Agent-token WS receives only `agent_room.message` and may send `client.agent_stream` fragments that fan out as ephemeral `server.agent_stream` frames to owner WS sessions. Owner clients must read bridge online state from native Matrix room state in the real `agent_room_id`: `io.direxio.agent.status` with state key `@agent:<server>` and content field `online`. The server publishes `online=true` only while Agent config is enabled and at least one agent-token WS gateway session is connected; startup/repair without a connected gateway publishes `online=false`. `sync.bootstrap` only returns `agent_room_id`; do not add `agent_online` back to bootstrap and do not emit `agent.presence` events. `agent.status`/`agents.status` are removed and must not be used. The real `agent_room_id` defaults to no system push for the portal owner through a room-level Matrix push rule with empty actions; preserve an existing explicit rule for that room.
