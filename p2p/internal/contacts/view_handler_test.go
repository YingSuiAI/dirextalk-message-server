...
func TestHandlersOwnOnlyContactsListAndReturnConcreteViews(t *testing.T) {
	...
	sort.Strings(names)
	if want := []string{"contacts.delete", "contacts.list", "contacts.requests.accept", "contacts.requests.delete", "contacts.requests.reject", "contacts.update"}; !reflect.DeepEqual(names, want) {
