...
func TestUpdateHandlerMapsSaveAndOperationErrors(t *testing.T) {
	existing := dirextalkdomain.ContactRecord{PeerMXID: "@alice:example.com", RoomID: "!direct:example.com", Status: "accepted"}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, apiErr := harness.module.Handlers()[actionUpdate](context.Background(), map[string]any{
				"room_id": existing.RoomID, "display_name": "Alice",
			})
			if result != nil || apiErr == nil || apiErr.Status != http.StatusInternalServerError || apiErr.Error != "internal error: update failed" {
				t.Fatalf("contacts.update failure = (%#v, %#v)", result, apiErr)
			}
			...
			for _, call := range harness.log.snapshot() {
				if call == "operation:contacts.update:accepted:!direct:example.com" {
