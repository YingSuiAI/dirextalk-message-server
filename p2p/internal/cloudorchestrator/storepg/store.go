...
func transitionCloudJob(ctx context.Context, tx *sql.Tx, jobID, planID, deploymentID, kind, stepID string, now int64, transition researchJobTransition) (cloudmodule.Job, error) {
	if jobID == "" || kind == "" || stepID == "" ||
		...
		((kind == "provision" || kind == "verify" || kind == "install" || kind == "destroy" || kind == "backup") && (planID == "" || deploymentID == "")) ||
		(kind != "connection_registration" && kind != "provision" && kind != "verify" && kind != "install" && kind != "destroy" && kind != "backup" && (planID == "" || deploymentID != "")) {
