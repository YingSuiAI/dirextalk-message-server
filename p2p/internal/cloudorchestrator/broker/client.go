...
// fixed typed commands: quote/request registration, deployment lifecycle
// evidence, and the narrow Worker task issue/observe contract. It is not an
// arbitrary AWS or Worker-control API proxy.
type Client struct {
	endpoint         *url.URL
...
// SubmitRegistration sends one already-signed connection registration
// verification envelope to the configured endpoint. A successful return means
// the receipt and every stack attestation field are strictly bound to that
// exact command and to the endpoint that returned it.
func (client *Client) SubmitRegistration(ctx context.Context, command RegistrationCommand) (RegistrationResult, error) {
	if client == nil || client.endpoint == nil || client.httpClient == nil {
		return RegistrationResult{}, newError("broker_client_unavailable", nil)
	}
	if ctx == nil {
		return RegistrationResult{}, newError("invalid_broker_context", nil)
	}
	if err := command.Validate(); err != nil {
	...
	body, err := json.Marshal(command)
	if err != nil || len(body) > maxRequestBytes {
		return RegistrationResult{}, newError("invalid_command", err)
	}
	request, err := http.NewRequestWithContext(ctx, http.MethodPost, client.endpoint.String(), bytes.NewReader(body))
	if err != nil {
		return RegistrationResult{}, newError("broker_request_unavailable", err)
	}
	request.Header.Set("Accept", "application/json")
	request.Header.Set("Cache-Control", "no-store")
	request.Header.Set("Content-Type", "application/json")
	...
	if err != nil {
		if errors.Is(err, context.DeadlineExceeded) || errors.Is(ctx.Err(), context.DeadlineExceeded) {
			return RegistrationResult{}, newError("broker_timeout", err)
		}
		return RegistrationResult{}, newError("broker_unavailable", err)
	}
	defer response.Body.Close()
	if response.StatusCode != http.StatusOK {
		if code := v2ErrorCode(response, client.maxResponseBytes); code != "" {
			return RegistrationResult{}, newHTTPError(code, response.StatusCode, nil)
		}
		return RegistrationResult{}, newHTTPError("broker_http_status", response.StatusCode, nil)
	}
	contentType, _, err := mime.ParseMediaType(response.Header.Get("Content-Type"))
	if err != nil || !strings.EqualFold(contentType, "application/json") {
		return RegistrationResult{}, newError("invalid_broker_content_type", err)
	}
	responseBody, err := readBounded(response.Body, client.maxResponseBytes)
	if err != nil {
		return RegistrationResult{}, err
	}
	result, err := decodeRegistrationResultJSON(responseBody)
	if err != nil {
		return RegistrationResult{}, newError("invalid_broker_response", err)
	}
	if err := ValidateRegistrationResult(command, result); err != nil {
		return RegistrationResult{}, newError("invalid_broker_response", err)
	}
	registrationEndpoint, err := parseBrokerEndpoint(result.Registration.BrokerCommandURL)
	if err != nil || registrationEndpoint.String() != client.endpoint.String() {
		return RegistrationResult{}, newError("invalid_broker_response", err)
	}
	return result, nil
}
