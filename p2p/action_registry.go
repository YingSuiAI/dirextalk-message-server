...
func (s *Service) actionHandlers() map[string]actionHandler {
	modules := []actionHandlerModule{
		...
		{name: "group-agent", handlers: map[string]actionHandler{"groups.agent.get": s.groupAgentGet, "groups.agent.update": s.groupAgentUpdate, "groups.agent.schedules": s.groupAgentSchedules}},
