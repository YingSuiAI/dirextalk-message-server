...
func seedManagementAcceptanceEvidence(t *testing.T, store *DatabaseStore, now time.Time) {
	...
	recipe.VolumeSlots = []cloudcontracts.RecipeVolumeSlotRequirementV1{{SlotID: "data", Purpose: "service data", ReadOnly: false}}
	recipe.DataSlots = []cloudcontracts.RecipeDataSlotRequirementV1{{SlotID: "knowledge", Purpose: "knowledge data", ReadOnly: true}}
	recipe.SecretSlots = []cloudcontracts.RecipeSecretSlotRequirementV1{{SlotID: "model", Purpose: "model credential", Delivery: cloudcontracts.SecretDeliveryFile}}
	...
	statements := []struct {
		...
		{`UPDATE p2p_cloud_deployments SET resource_status='retained_tracked' WHERE deployment_id='deployment-destroy-0001'`, nil},
		{`UPDATE p2p_cloud_deployment_resources SET resource_status='retained_tracked' WHERE deployment_id='deployment-destroy-0001'`, nil},
		...
		{`INSERT INTO p2p_cloud_service_readiness_tasks(task_id,execution_id,deployment_id,service_id,cloud_connection_id,instance_id,recipe_execution_manifest_digest,install_evidence_digest,artifact_digest,semantic_probe_json,semantic_expectation_digest,task_status,purpose,job_id,semantic_evidence_digest,stack_observation_digest,created_at,updated_at) VALUES('readiness-management-install','execution-management-install-0001','deployment-destroy-0001','service-destroy-0001','connection-destroy-0001','i-0123456789abcdef0',$1,$2,$3,$4,$5,'succeeded','install','',$5,$6,$7,$7)`, []any{manifestDigest, "sha256:cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc", manifest.ArtifactDigest, string(semanticProbeJSON), semanticProbe.BodySHA256, "sha256:dddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddddd", now.UnixMilli()}},
