...
func TestContactStorageRecordExcludesFacadeFields(t *testing.T) {
	contactType := reflect.TypeOf(contactStorageRecord{})
	for _, fieldName := range []string{"Operation", "Conversation"} {
		if _, ok := contactType.FieldByName(fieldName); ok {
			t.Fatalf("durable contact storage record must not expose response-only field %s", fieldName)
		}
	}
}
