...
func TestReleaseActionsOwnerAuthTransportAndPersistence(t *testing.T) {
	...
	if controller.statusRequest.CurrentVersion != "v1.0.1" || controller.statusRequest.CurrentSchemaVersion != 1 || controller.statusRequest.CurrentSchemaCompatVersion != 1 || controller.statusRequest.ClientVersion != "v2.3.4" {
...
func TestReleaseStatusKeepsLocalCurrentAndClientVersionsAuthoritative(t *testing.T) {
	...
	status := mustHandle[map[string]any](t, service, "release.v1.status", nil)
	if status["current_version"] != "v1.0.1" || status["client_version"] != "v2.3.4" {
