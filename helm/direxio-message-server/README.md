...
Direxio Message Server Matrix Homeserver
...
You'll need to create the following database before starting Direxio Message Server (see [installation](../../docs/installation/manual/2_database.md)):
...
* <https://github.com/YingSuiAI/direxio-message-server>
## Requirements
...
|-----|------|---------|-------------|
| image.repository | string | `"ghcr.io/yingsuiai/direxio-message-server"` | Docker repository/image to use |
...
| extraArgs | list | `[]` | Add additional arguments to the direxio-message-server command |
| extraVolumes | list | `[]` | Add additional volumes to the Direxio Message Server Pod |
| extraVolumeMounts | list | `[]` | Configure additional mount points volumes in the Direxio Message Server Pod |
...
| dendrite_config.global.server_name | string | `""` | **REQUIRED** Servername for this Direxio Message Server deployment. |
| dendrite_config.global.private_key | string | `"/etc/direxio-message-server/secrets/signing.key"` | The private key to use. (**NOTE**: This is overriden in Helm) |
...
| dendrite_config.global.disable_federation | bool | `false` | Disable federation. Direxio Message Server will not be able to make any outbound HTTP requests to other servers and the federation API will not be exposed. |
...
| dendrite_config.global.jetstream.topic_prefix | string | `"Direxio Message Server"` | The prefix for JetStream streams |
...
| dendrite_config.global.report_stats.enabled | bool | `false` | Configures phone-home statistics reporting. These statistics contain the server name, number of active users and some information on your deployment config. We use this information to understand how Direxio Message Server is being used in the wild. |
...
| dendrite_config.federation_api.disable_http_keepalives | bool | `false` | Prevents Direxio Message Server from keeping HTTP connections open for reuse for future requests. Connections will be closed quicker but we may spend more time on TLS handshakes instead. |
...
| dendrite_config.sync_api.real_ip_header | string | `"X-Real-IP"` | This option controls which HTTP header to inspect to find the real remote IP address of the client. This is likely required if Direxio Message Server is running behind a reverse proxy server. |
...
| dendrite_config.sync_api.search.language | string | `"en"` | The language most likely to be used on the server - used when indexing, to ensure the returned results match expectations. A full list of possible languages can be found [here](https://github.com/YingSuiAI/direxio-message-server/blob/76db8e90defdfb9e61f6caea8a312c5d60bcc005/internal/fulltext/bleve.go#L25-L46) |
