...
Dirextalk Message Server Matrix Homeserver
...
|-----|------|---------|-------------|
| image.repository | string | `"ghcr.io/yingsuiai/dirextalk-message-server"` | Docker repository/image to use |
...
| extraArgs | list | `[]` | Add additional arguments to the dirextalk-message-server command |
| extraVolumes | list | `[]` | Add additional volumes to the Dirextalk Message Server Pod |
| extraVolumeMounts | list | `[]` | Configure additional mount points volumes in the Dirextalk Message Server Pod |
...
| dendrite_config.global.server_name | string | `""` | **REQUIRED** Servername for this Dirextalk Message Server deployment. |
| dendrite_config.global.private_key | string | `"/etc/dirextalk-message-server/secrets/signing.key"` | The private key to use. (**NOTE**: This is overriden in Helm) |
...
| dendrite_config.global.disable_federation | bool | `false` | Disable federation. Dirextalk Message Server will not be able to make any outbound HTTP requests to other servers and the federation API will not be exposed. |
...
| dendrite_config.global.jetstream.topic_prefix | string | `"Dirextalk Message Server"` | The prefix for JetStream streams |
...
| dendrite_config.sync_api.real_ip_header | string | `"X-Real-IP"` | This option controls which HTTP header to inspect to find the real remote IP address of the client. This is likely required if Dirextalk Message Server is running behind a reverse proxy server. |
...
| dendrite_config.sync_api.search.language | string | `"en"` | The language most likely to be used on the server - used when indexing, to ensure the returned results match expectations. A full list of possible languages can be found [here](https://github.com/YingSuiAI/dirextalk-message-server/blob/76db8e90defdfb9e61f6caea8a312c5d60bcc005/internal/fulltext/bleve.go#L25-L46) |
