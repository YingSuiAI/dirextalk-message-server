...
func NotifyUserCountsAsync(ctx context.Context, pgClient pushgateway.Client, localpart string, serverName spec.ServerName, db storage.UserDatabase) error {
	...
	go func() {
		...
		for _, pusherDevice := range pusherDevices {
			...
			for _, pushKey := range resp.Rejected {
				if pushKey != pusherDevice.Device.PushKey {
					continue
				}
				if err := db.RemovePusher(ctx, pusherDevice.Device.AppID, pushKey, localpart, serverName); err != nil {
					log.WithFields(log.Fields{
						"localpart": localpart,
						"app_id":    pusherDevice.Device.AppID,
						"pushkey":   pushKey,
					}).WithError(err).Error("Failed to remove rejected HTTP pusher")
				}
			}
		}
	}()
