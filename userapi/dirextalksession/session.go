...
func NewIssuer(userAPI userapi.UserInternalAPI, serverName spec.ServerName, defaultDeviceID, deviceDisplayName string) *Issuer {
	return &Issuer{
		userAPI:           userAPI,
		serverName:        serverName,
		defaultDeviceID:   strings.TrimSpace(defaultDeviceID),
		deviceDisplayName: strings.TrimSpace(deviceDisplayName),
	}
}

func (i *Issuer) DeactivateAccount(ctx context.Context, localpart string) error {
...
func (i *Issuer) EnsureMatrixSession(ctx context.Context, userID, displayName, avatarURL, requestedDeviceID string, revokeExistingDevices bool) (string, error) {
	...
	deviceID := cleanDeviceID(requestedDeviceID, i.defaultDeviceID)
	deviceName := i.deviceDisplayName
...
func (i *Issuer) UpdateMatrixProfile(ctx context.Context, userID, displayName, avatarURL string) error {
	localpart, serverName, err := gomatrixserverlib.SplitID('@', userID)
	if err != nil {
		return err
	}
...
func (i *Issuer) updateMatrixProfile(ctx context.Context, localpart string, serverName spec.ServerName, displayName, avatarURL string) error {
...
func cleanDeviceID(deviceID, fallback string) string {
