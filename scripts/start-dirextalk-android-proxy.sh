...
PROXY_PY="${ROOT_DIR}/scripts/dirextalk_domain_proxy.py"
CERT_DIR="${ROOT_DIR}/.local-certs/dirextalk"
RUN_DIR="${RUN_DIR:-/tmp/dirextalk-proxy}"
ADB="${ADB:-}"
...
EOF
  ...
  if [[ ! -f "${CERT_DIR}/ca.key" || ! -f "${CERT_DIR}/ca.pem" ]]; then
    log "generating local Dirextalk CA"
    openssl genrsa -out "${CERT_DIR}/ca.key" 4096
    openssl req -x509 -new -nodes -key "${CERT_DIR}/ca.key" -sha256 -days 3650 \
      -out "${CERT_DIR}/ca.pem" \
      -subj "/C=CN/O=Dirextalk Local Dev/CN=Dirextalk Local Dev CA"
  fi
  ...
  openssl req -new -key "${CERT_DIR}/server.key" -out "${CERT_DIR}/server.csr" \
    -subj "/C=CN/O=Dirextalk Local Dev/CN=a.ai" >/dev/null 2>&1
...
cp -f /system/etc/hosts /system/etc/hosts.dirextalk.bak 2>/dev/null || true
...
set -e
printf '127.0.0.1       localhost\n::1             ip6-localhost\n127.0.0.1       a.ai b.ai c.ai host.docker.internal\n' > /data/local/tmp/dirextalk-hosts
chmod 0644 /data/local/tmp/dirextalk-hosts
rm -rf /data/local/tmp/dirextalk-cacerts
cp -a /system/etc/security/cacerts /data/local/tmp/dirextalk-cacerts
cp -f /data/local/tmp/${hash}.0 /data/local/tmp/dirextalk-cacerts/${hash}.0
chmod 0644 /data/local/tmp/dirextalk-cacerts/${hash}.0
mount -o bind /data/local/tmp/dirextalk-hosts /system/etc/hosts
mount -o bind /data/local/tmp/dirextalk-cacerts /system/etc/security/cacerts
EOF
)"
