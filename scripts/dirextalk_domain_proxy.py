...
class DirextalkDomainProxy(BaseHTTPRequestHandler):
...
def main():
    ...
    port = args.port
    server = ThreadingHTTPServer(("127.0.0.1", port), DirextalkDomainProxy)
    scheme = "http"
    if args.cert and args.key:
        ...
        scheme = "https"
    print(f"dirextalk domain proxy listening on {scheme}://127.0.0.1:{port}", flush=True)
