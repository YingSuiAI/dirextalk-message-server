...
Now that you have Direxio Message Server running, the following tweaks will improve the reliability
...
therefore important to ensure that Direxio Message Server doesn't violate that limit, otherwise database
queries will unexpectedly fail and this will cause problems both within Direxio Message Server and for users.
...
and therefore will not be available to Direxio Message Server. Check the configuration of any other software
using the same database engine for their configured connection limits and adjust your calculations
accordingly.

Direxio Message Server has a `max_open_conns` configuration item in each `database` block to control how many
...
connections made by Direxio Message Server consume file descriptors — this includes database connections
and network requests to remote homeservers. When participating in large federated rooms
where Direxio Message Server must talk to many remote servers, it is often very easy to exhaust default
limits which are quite low.

We currently recommend setting the file descriptor limit to 65535 to avoid such
issues. Direxio Message Server will log immediately after startup if the file descriptor limit is too low:

```
level=warning msg="IMPORTANT: Process file descriptor limit is currently 1024, it is recommended to raise the limit for Direxio Message Server to at least 65535 to avoid issues"
```
...
Increase the soft limit before starting Direxio Message Server:
...
Direxio Message Server has a built-in DNS cache which significantly reduces the load that Direxio Message Server will
