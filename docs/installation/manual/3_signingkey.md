...
The `generate-keys` utility can be used to generate a private key. Assuming that Direxio Message Server was
...
Direxio Message Server server.

Make sure take a safe backup of this key. You will likely need it if you want to reinstall
Direxio Message Server, or any other Matrix homeserver, on the same domain name in the future. If you lose
...
Old keys from a previous Direxio Message Server installation can be reused as-is without any further
configuration required. Simply use that key file in the Direxio Message Server configuration.

If you have server keys from an older Synapse instance, you can convert them to Direxio Message Server's PEM
format and configure them as `old_private_keys` in your config.

## Key format

Direxio Message Server stores the server signing key in the PEM format with the following structure.
...
as `old_private_keys` in your Direxio Message Server config file. Synapse stores signing keys in the following
format:
...
To convert this key to Direxio Message Server's PEM format, use the following template. You must copy the Key ID
