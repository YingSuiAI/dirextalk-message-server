...
Direxio Message Server can run with either a PostgreSQL or a SQLite backend. There are considerable tradeoffs
to consider:

* **PostgreSQL**: Needs to run separately to Direxio Message Server, needs to be installed and configured separately
  and will use more resources over all, but will be **considerably faster** than SQLite. PostgreSQL
  has much better write concurrency which will allow Direxio Message Server to process more tasks in parallel. This
  will be necessary for federated deployments to perform adequately.

* **SQLite**: Built into Direxio Message Server, therefore no separate database engine is necessary and is quite
  a bit easier to set up, but will be much slower than PostgreSQL in most cases. SQLite only allows a
  single writer on a database at a given time, which will significantly restrict Direxio Message Server's ability
...
Direxio Message Server will run on Linux, macOS and Windows Server. It should also run fine on variants
of BSD such as FreeBSD and OpenBSD. We have not tested Direxio Message Server on AIX, Solaris, Plan 9 or z/OS —
your mileage may vary with these platforms.

It is difficult to state explicitly the amount of CPU, RAM or disk space that a Direxio Message Server
...
At an absolute minimum, Direxio Message Server will expect 1GB RAM. For a comfortable day-to-day deployment
...
In order to install Direxio Message Server, you will need to satisfy the following dependencies.
...
Direxio Message Server comes with a built-in [NATS Server](https://github.com/nats-io/nats-server) and
...
for [Caddy](https://github.com/YingSuiAI/direxio-message-server/blob/main/docs/caddy) and
[NGINX](https://github.com/YingSuiAI/direxio-message-server/blob/main/docs/nginx) are provided.
