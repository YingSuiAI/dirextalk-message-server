...
- [ ] Make P2P projector batching/backpressure configurable after confirming idempotency and event ordering requirements. Status: reviewed and intentionally deferred. The durable JetStream consumer currently processes one roomserver output at a time with manual ack/retry. Redaction projection and reaction target resolution now use indexed post/comment lookups instead of full content scans, and projected P2P delta events are deduplicated by source event/action. Raising batch/concurrency remains blocked until per-room ordering requirements and retry/backoff visibility are addressed.
...
- Keep `P2POutputRoomEventConsumer` batch size at 1 until per-room ordering and consumer lag/backoff visibility are explicit.
...
- Before enabling batching, add retry/backoff metrics around durable consumer lag and preserve strict per-room ordering.
