...
| Restart and PostgreSQL-backed projection tests are thin for refactor-sensitive paths. | `p2p/service_member_persistence.go`: `saveMember`; `p2p/service_channels.go`: `saveChannel`, `refreshStoredChannelCounts`; `p2p/service_contacts.go`: `saveContact`; `p2p/service_channel_backfill_test.go`: channel-content backfill reopen fixture; `p2p/service_system_report_test.go`: report submit/reload fixture; `p2p/business_state_test.go`: `TestStoredMemberRolesAndMutesSurviveReload`, `TestStoredChannelCountsSurviveReload`; `p2p/storage_test.go`: storage tests | Channel content backfill, report submission, member roles/mutes, and channel counts now have PostgreSQL reopen coverage. Add DB restart tests for remaining contact deletion/reactivation before moving storage boundaries. |
...
7. Phase D: move lower-level adapters in dependency-safe order.
   ...
   - Next Phase D slice: add remaining DB/restart projection fixtures for contact deletion/reactivation, then continue with narrower storage/projection boundaries.
