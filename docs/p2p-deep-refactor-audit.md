...
| History reader parity is only partially enforced. | `internal/dirextalkmcp/message_summary_test.go`: shared ordinary-message summary helper fixtures; `internal/dirextalkmatrix/message_reader_test.go`: HTTP ordinary-message and channel-content fixtures; `p2p/matrixhistory/reader_test.go`: facade compatibility fixture; `syncapi/agenthistory/reader.go`: `Reader.ListOrdinaryMessages`; `p2p/mcp_api.go`: `mcpMessagesList` | Add DB-backed fixtures proving sync-backed reader iteration, visibility filtering, and pagination produce the same summaries as the shared helper. |
...
7. Phase D: move lower-level adapters in dependency-safe order.
   ...
   - Next Phase D slice: add DB-backed sync history fixtures for iteration/visibility behavior before changing reader storage access, then continue with narrower storage/projection boundaries.
