...
| Restart and PostgreSQL-backed projection tests are thin for refactor-sensitive paths. | `p2p/service_member_persistence.go`: `saveMember`; `p2p/service_channels.go`: `saveChannel`, `refreshStoredChannelCounts`; `p2p/service_contacts.go`: `saveContact`; `p2p/service_channel_backfill_test.go`: channel-content backfill reopen fixture; `p2p/storage_test.go`: storage tests | Channel content backfill now has PostgreSQL reopen coverage. Add DB restart tests for remaining member roles/mutes, channel counts, contact deletion/reactivation, and report submission before moving storage boundaries. |
...
7. Phase D: move lower-level adapters in dependency-safe order.
   ...
   - Next Phase D slice: add remaining DB/restart projection fixtures for member roles/mutes, channel counts, contacts, and reports, then continue with narrower storage/projection boundaries.
