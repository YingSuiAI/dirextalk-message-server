...
Resolved compatibility timing after MCP-D:
...
| Storage adapter aliases | `p2p/storage_adapter.go`: `DatabaseStore`, `NewDatabaseStore`, `NewUnmigratedDatabaseStore`; `p2p/storage/storage.go`: `DatabaseStore`; `p2p/storage/storage_migrations.go`: `DatabaseStore.migrate`, `execMigrationStatements`; `p2p/service.go`: `Store` | Root package alias over the real storage package plus focused p2p store interfaces composed by `Store`. | Phase E now composes `Store` from focused domain interfaces with contract tests. Do not move storage tables blindly because some are product source of truth and others are projections. |
...
| Matrix history adapter | `internal/dirextalkmcp/types.go`: `MessageSummary`, `Page`, `MessagePageResult`, `SortMessageSummaries`, `Event`, `InTimeRange`, `InPage`, `FormatTime`, `OrdinaryMessageSummary`; `internal/dirextalkmatrix/message_reader.go`: `HTTPMessageReader`, `NewHTTPMessageReader`, `ListOrdinaryMessages`, `ListChannelContent`; `internal/dirextalkmatrix/types.go`: neutral aliases/helpers; `p2p/matrixhistory/types.go` and `p2p/matrixhistory/reader.go`: compatibility aliases/wrapper; `p2p/matrix_history_reader.go`: `HTTPMatrixHistoryReader`, `NewHTTPMatrixHistoryReader`; `p2p/mcp/types.go`: compatibility aliases; `syncapi/agenthistory/reader.go`: `Reader`, `ListOrdinaryMessages` | Provides HTTP and sync-backed history readers for MCP and channel backfill. | Phase D moved HTTP Matrix history reads to `internal/dirextalkmatrix`; HTTP and sync-backed ordinary message summaries share `internal/dirextalkmcp.OrdinaryMessageSummary`; p2p remains the service adapter. |
| Product Matrix state builder | `internal/dirextalkstate/state.go`: `StateEvent`, `DirectRoomProfileInput`, `GroupProfile`, `DirectRoomProfile`, `GroupRoomProfile`, `ChannelRoomProfile`, `MemberPolicyState`, `JoinRequestState`; `p2p/product_native.go`: `roomStateEvent`, `roomProfileForDirect`, `accountDeletedDirectProfile`, `roomProfileForGroup`, `roomProfileForChannel`; `p2p/service_channels.go`: `publishMemberPolicyState`, `publishJoinRequestState`; `p2p/service_groups.go`: `groupStateEvent`; `p2p/service_account_delete.go`: `publishAccountDeletedDirectState` | Builds neutral Dirextalk Matrix state-event content for room profile, member policy, and join-request facts. | Phase D moved schema/content construction below the p2p facade; p2p still decides when to publish state through the transport. |
| Shared product domain values | `internal/dirextalkdomain/types.go`: portal/profile/agent config, channel/member/group/conversation, block, call, favorite, follow, reaction, report, invite grant, P2P event, event bounds records, durable contact/group/channel post/channel comment records; `internal/dirextalkdomain/conversation.go`: conversation kinds/lifecycles/helpers; `p2p/domain/types.go`, `p2p/domain/conversation.go`: compatibility aliases plus response-shaped contact/group/channel post/comment facades; `p2p/storage/aliases.go`: direct aliases to owning internal value packages where possible | Provides shared value records and pure helper behavior without forcing lower-level packages through `p2p/domain`. | Phase D moved pure shared records/helpers into `internal/dirextalkdomain`; Phase E split durable contact/group/channel post/channel comment records from p2p response facades while leaving `Operation`/`ConversationView` enrichment in p2p. |
| Plugin record values | `internal/dirextalkplugin/types.go`: `CatalogEntry`, `Instance`, `Job`, `Secret`; `p2p/domain/types.go`: compatibility aliases; `p2p/service_plugins.go`: plugin lifecycle/config/orchestration; `p2p/storage/storage_plugins.go`: durable plugin rows | Keeps non-Agent plugin record shapes below p2p while leaving Docker/plugin orchestration in p2p. | Phase D moved plugin record values into `internal/dirextalkplugin`; p2p still owns plugin actions, runner calls, secret handling behavior, and Native Agent/plugin separation. |
...
`p2p.Service` should become an adapter that supplies `Store`, `Transport`, Matrix history reader, profile resolver, owner context, and `mcp_blocked_room_ids` behavior to `internal/dirextalkmcp`. `p2p/mcp_api.go` keeps the p2p-owned capability implementation behind the unified service; `p2p/action_registry_mcp.go` and fixed body-action registration have been removed.
...
| MCP capability business logic was duplicated between body-action handlers and Native Agent tools. | `internal/dirextalkmcp/service.go`: `Service`, `Tools`, action constants, schemas; `internal/dirextalkmcp/pagination.go`: pagination/cursor helpers; `internal/dirextalkmcp/types.go`: DTOs and shared errors; `p2p/mcp_service.go`: `p2pDirextalkMCPInvoker`, `p2pDirextalkMCPRoomAuthorizer`; `p2p/routing_mcp.go`: standard MCP JSON-RPC transport; `p2p/native_agent_runner.go`: `nativeAgentTools`; `p2p/mcp_unified_service_test.go`: unified service/tool tests | The same Dirextalk MCP capability can drift across external `/mcp` transport and internal Native Agent tools, including schemas, room blocking, pagination, and write-path guarantees. | Phase MCP-B created `internal/dirextalkmcp`; Phase MCP-D removed fixed `mcp.*` body actions, leaving `POST /mcp` and Native Agent Dirextalk tools on one service. |
...
| Matrix message history filtering and summary formatting is partly shared between HTTP and sync-backed readers. | `internal/dirextalkmcp/types.go`: `MessageSummary`, `FormatTime`, `InTimeRange`, `InPage`, `OrdinaryMessageSummary`; `internal/dirextalkmatrix/message_reader.go`: `HTTPMessageReader.ListOrdinaryMessages`, `HTTPMessageReader.ListChannelContent`; `internal/dirextalkmatrix/message_reader_test.go`: HTTP ordinary-message and channel-content fixtures; `syncapi/agenthistory/reader.go`: `Reader.ListOrdinaryMessages`; `syncapi/agenthistory/reader_test.go`: DB-backed ordinary-message iteration, local-hide filtering, cursor, and roomserver-backed history-visibility fixtures; `p2p/service_channel_backfill.go`: `backfillJoinedChannelContent`, `channelContentBackfillWeight`; `p2p/service_channel_backfill_test.go`: dependency-order and PostgreSQL reopen channel-content backfill fixtures; `p2p/matrixhistory/types.go`: compatibility aliases | Ordinary-message filtering, pagination, timestamp formatting, and sender fields now share one helper. Sync-backed ordinary-message reads now scan multiple topology batches so product events filtered out of ordinary chat do not truncate results. Channel-content backfill now projects posts before comments before reactions so timestamp skew cannot drop reaction counts, and its projected counts survive service/store reopen. Agent history has DB-backed coverage for local hide and roomserver history visibility. | Keep HTTP and sync readers on neutral packages; add broader DB/restart projection coverage before moving more history/storage boundaries. |
| Channel/group member counts are recomputed and persisted through several service methods. | `internal/dirextalkprojection/counts.go`: `ProductMemberCounts`; `p2p/service_channels.go`: `channelWithCurrentCounts`, `refreshStoredChannelCounts`, `refreshStoredGroupCounts`, `refreshChannelCountsLocked`, `refreshGroupCountsLocked`; `p2p/conversation_service.go`: `hydrateConversationMembership`; `p2p/service_member_persistence.go`: `saveMember`; `p2p/service_member_invites.go`: `refreshRoomMembers`; `p2p/projector.go`: `ProjectRoomEvent` | Count updates can differ depending on whether the path is command-side, projection-side, or refresh-side. | Continue moving projection-only calculations behind `internal/dirextalkprojection` before changing storage writes. |
| Product room state-event construction was split by product type. | `internal/dirextalkstate/state.go`: `DirectRoomProfile`, `GroupRoomProfile`, `ChannelRoomProfile`, `MemberPolicyState`, `JoinRequestState`; `p2p/product_native.go`: `roomStateEvent`, `roomProfileForDirect`, `accountDeletedDirectProfile`, `roomProfileForGroup`, `roomProfileForChannel`; `p2p/service_channels.go`: `channelStateEvent`, `publishChannelState`, `publishChannelHistoryVisibilityState`, `publishMemberPolicyState`, `publishJoinRequestState`; `p2p/service_groups.go`: `groupStateEvent`, `publishGroupState`; `p2p/service_account_delete.go`: `publishAccountDeletedDirectState` | The Matrix state event type/key/content conventions now have one lower-level implementation for room profile, member policy, and join request content while p2p keeps publish timing. | Continue moving only schema/content helpers below p2p; keep action orchestration, sender selection, validation, and transport publishing in p2p. |
...
| Product state event content builders. | `internal/dirextalkstate/state.go`: `DirectRoomProfile`, `GroupRoomProfile`, `ChannelRoomProfile`, `MemberPolicyState`, `JoinRequestState`; `p2p/product_native.go`: `roomStateEvent`, `roomProfileForDirect`, `accountDeletedDirectProfile`, `roomProfileForGroup`, `roomProfileForChannel`; `p2p/service_channels.go`: `publishMemberPolicyState`, `publishJoinRequestState`; `p2p/service_groups.go`: `groupStateEvent`; `p2p/service_account_delete.go`: `publishAccountDeletedDirectState` | Moved content builders into `internal/dirextalkstate`; p2p still decides when to publish and which Matrix sender to use. | Keep Matrix state event schema centralized and avoid adding new ad hoc state content maps in service handlers. |
...
| Product state builders must stay below the p2p facade. | `internal/dirextalkstate/state.go`: imports `internal/dirextalkdomain` and `internal/productpolicy`; `p2p/product_native.go`: adapts `dirextalkstate.StateEvent` to `RoomStateEvent`; `p2p/service_channels.go`: calls `dirextalkstate.MemberPolicyState` and `dirextalkstate.JoinRequestState` | If `internal/dirextalkstate` imports `p2p`, state schema helpers would cycle with p2p service handlers and transport aliases. | Keep `rg "github.com/YingSuiAI/dirextalk-message-server/p2p" internal/dirextalkstate` empty; use neutral DTOs or `internal/dirextalkdomain` records only. |
| Shared value packages must stay below p2p. | `internal/dirextalkdomain/*`: shared records/helpers; `internal/dirextalkplugin/*`: plugin record shapes; `p2p/domain/*`: aliases and response-shaped facades; `p2p/storage/aliases.go`: direct internal aliases for durable values | If shared record packages import p2p, storage, transport, MCP, and projection packages regain facade dependencies and future storage/interface splits become cycle-prone. | Keep `rg "github.com/YingSuiAI/dirextalk-message-server/p2p" internal/dirextalkdomain internal/dirextalkplugin` empty. |
...
| Agent-token scope must remain narrow. | `p2p/serviceapi/actions.go`: `AgentAction`; `p2p/service.go`: `Service.Authorize`; `p2p/routing_test.go`: `TestAgentTokenCanOnlyCallAgentBootstrapAndStandardMCP`; `p2p/realtime_ws.go`: `createRealtimeWSTicketForToken`; `p2p/action_registry_test.go`: `TestFixedMCPBodyActionsAreRemovedFromProductRegistry` | Agent token must only call product body-action `agent.matrix_session.create` and standard `POST /mcp`; it must not create WS tickets, call owner product actions, or call removed fixed `mcp.*` body actions. | Generate tests over all registered actions from the action spec, not only selected examples. |
...
| Standard MCP HTTP endpoint is a deliberate product route exception. | `AGENTS.md`: no-URL-shaped-product-endpoint rule with explicit MCP exception; `docs/current-project-documentation.md`: `POST /mcp` endpoint contract; `p2p/routing_mcp.go`: standard MCP JSON-RPC transport; `p2p/native_agent_runner.go`: Native Agent tools generated from the same registry; `p2p/nativeagent/native_agent_eino_mcp.go`: existing MCP client transport use | `POST /mcp` changes the contract from body-action-only product capability access for external MCP clients. | Phase MCP-C pinned endpoint path, first-version `agent_token` auth, Origin/token handling, GET 405, and no bearer forwarding. Phase MCP-D removed the fixed `mcp.*` body actions from product routing. |
...
| History reader parity still needs broader fixture coverage. | `internal/dirextalkmcp/message_summary_test.go`: shared ordinary-message summary helper fixtures; `internal/dirextalkmatrix/message_reader_test.go`: HTTP ordinary-message and channel-content fixtures; `p2p/matrixhistory/reader_test.go`: facade compatibility fixture; `syncapi/agenthistory/reader.go`: `Reader.ListOrdinaryMessages`; `syncapi/agenthistory/reader_test.go`: DB-backed sync iteration, local-hide filtering, cursor pagination, and RSAPI history-visibility fixtures; `p2p/service_channel_backfill_test.go`: channel-content dependency-order and PostgreSQL reopen fixtures; `p2p/mcp_api.go`: `mcpMessagesList` | Add broader DB/restart projection parity before changing storage boundaries further. |
| Restart and PostgreSQL-backed projection tests are thin for refactor-sensitive paths. | `p2p/service_member_persistence.go`: `saveMember`; `p2p/service_channels.go`: `saveChannel`, `refreshStoredChannelCounts`; `p2p/service_contacts.go`: `saveContact`; `p2p/service_channel_backfill_test.go`: channel-content backfill reopen fixture; `p2p/service_system_report_test.go`: report submit/reload fixture; `p2p/business_state_test.go`: `TestStoredMemberRolesAndMutesSurviveReload`, `TestStoredChannelCountsSurviveReload`, `TestDeletedContactRequestRestoresOriginalRoomAfterReload`; `p2p/storage_test.go`: storage tests | Channel content backfill, report submission, member roles/mutes, channel counts, and contact deletion/reactivation now have PostgreSQL reopen coverage. Keep these tests before moving storage boundaries. |
...
| Unified MCP service parity is covered for the service boundary and still needs broader fixture parity. | `internal/dirextalkmcp/service_test.go`: registry/tool/authorization tests; `p2p/mcp_unified_service_test.go`: direct p2p adapter and Native Agent tool invocation through the same service; `p2p/mcp_http_test.go`: JSON-RPC `tools/call` invocation through the same service; `p2p/mcp_api_test.go`: behavior coverage for contacts, rooms, messages, members, channel posts/comments, blocked rooms, and pagination | Boundary tests prove `POST /mcp`, direct p2p adapter calls, and Native Agent Dirextalk tools invoke the same `internal/dirextalkmcp` service. | Add future fixture parity across every capability before changing MCP DTOs or pagination behavior. |
...
Current Phase E checkpoint evidence:

- Store split: `p2p/service.go:Store` now embeds focused interfaces from `p2p/service_portal_store.go`, `p2p/service_channels.go`, `p2p/service_channel_content.go`, `p2p/service_contacts.go`, `p2p/service_groups.go`, `p2p/service_member_persistence.go`, `p2p/conversation_service.go`, `p2p/product_native.go`, `p2p/service_plugins.go`, `p2p/service_system_report.go`, `p2p/service_social_calls.go`, `p2p/service_blocks.go`, and `p2p/service_profile_sync.go`.
- Interface coverage: `p2p/store_interface_contract_test.go` proves a composed focused store satisfies `Store`; `p2p/*_store_contract_test.go` files prove each focused interface can compile independently.
...
| Storage surface | Owner/source-of-truth classification | Move rule |
| --- | --- | --- |
| `p2p_portal`, `portalState`, `ownerProfile`, `agentConfig` | Source-of-truth local owner setup, Matrix session metadata, durable system/agent room ids, and Native Agent config. | Keep writes in portal/auth/profile/agent config orchestration; any lower-level package must receive small interfaces and must not persist model API keys. |
| `p2p_contacts`, `contactStorageRecord` | Mixed: direct relationship lifecycle/remark/delete recovery are product source-of-truth; display/avatar/domain values are Matrix/profile projections unless explicitly overridden. | Keep response-only `Operation`/`Conversation` in p2p facade; keep old-room identity durable for delete/reactivate flows. |
| `p2p_groups`, `groupStorageRecord` | Mixed: owner-created group profile/mute counts are product projections from Matrix state/member facts; local mute is product local state. | Do not move group writes without preserving Matrix state projection and reload tests for roles/mutes. |
| `p2p_channels`, `channel` | Mixed: channel metadata/counts are Matrix/product projections; local mute is product local state; visibility/join policy control public lookup behavior. | Keep public lookup read-only and preserve channel count/reload tests before moving writes. |
| `p2p_channel_posts`, `channelPostStorageRecord` | Projection backed by Matrix channel post events and redactions. Stored `comment_count` is a denormalized projection. | Keep ordinary channel chat separate from posts; rebuild via channel content backfill before changing table ownership. |
| `p2p_channel_comments`, `channelCommentStorageRecord` | Projection backed by Matrix channel comment events and redactions. Reply/mentions/media fields are event-derived durable projection fields. | Keep reaction counts out of the durable record facade boundary; counts come from `p2p_reactions` enrichment. |
| `p2p_reactions`, `reactionRecord` | Product projection backed by Matrix reaction events; active flag is latest projected state. | Keep target lookup and backfill dependency order tests before moving reaction writes. |
| `p2p_members`, `memberRecord` | Projection/source mix: Matrix membership is the joined fact; product role, mute, requester-node URL, and invite/join-request metadata are product facts projected into/read from Matrix state and callbacks. | Do not make public joins appear joined before Matrix join. Preserve requester-node URL and role/mute reload tests. |
| `p2p_conversations`, `conversationRecord` | Projection/read model derived from contacts/groups/channels/system reports and Matrix activity. | Keep it as a read model; updates can move only behind projection helpers with conflict tests. |
| `p2p_reports`, `reportRecord` | Source-of-truth owner-directed report submission plus Matrix system notice event id/timestamp. | Keep report row and system-room notice coupled; preserve reload coverage. |
| `p2p_plugins`, `p2p_plugin_jobs`, `p2p_plugin_secrets` | Source-of-truth for non-Agent plugin management/runtime state. | Do not mix Native Agent config back into plugin storage; keep Agent-as-plugin compatibility deletion isolated. |
| `p2p_calls`, `callRecord` | Product call state record backed by product actions/events. | Preserve lifecycle state transitions before moving call storage. |
| `p2p_favorites`, `p2p_follows`, `p2p_read_markers`, `p2p_blocks` | Local product source-of-truth state. Blocks also constrain contact/MCP behavior. | Keep local-only semantics explicit; do not infer these from Matrix timeline state. |
| `p2p_events` | Internal product event log with pruning/dedupe; not a client contract by itself. | Keep bounded retention and dedupe behavior if event storage moves. |
...
- `POST /mcp`, p2p adapter calls, and Native Agent Dirextalk tools calling the same service;
- tests proving adapter/tool/transport parity and unchanged response behavior.
...
Status after Phase MCP-D verification: Phase B, Phase C, Phase MCP-B, Phase MCP-C, and fixed MCP body-action removal have implementation and regression coverage in this branch. Standard `POST /mcp`, Native Agent tools, and multi-node flows are the current supported MCP surfaces.
...
4. Phase MCP-B: build the unified internal Dirextalk MCP capability service.
   ...
   - Make p2p MCP adapters call `internal/dirextalkmcp`.
   - Make Native Agent Dirextalk tools generated from and invoked through the same service.
   - Preserve existing MCP capability response behavior for current tests.
...
7. Phase D: move lower-level adapters in dependency-safe order.
   ...
   - Completed sixth Phase D slice: HTTP and sync-backed ordinary message readers now share `internal/dirextalkmcp.OrdinaryMessageSummary` for product-event filtering, pagination, sender identity fields, and timestamp formatting.
   - Completed seventh Phase D slice: `syncapi/agenthistory.Reader.ListOrdinaryMessages` now has PostgreSQL-backed fixtures for filtered-event iteration, local hidden-event visibility, and stable cursor pagination; the reader scans additional topology batches when product events are filtered from ordinary chat results.
   - Completed eighth Phase D slice: channel-content backfill now has a dependency-order fixture and projects posts before comments before reactions, so reaction counts survive timestamp skew in historical Matrix events.
   - Completed ninth Phase D slice: `syncapi/agenthistory.Reader.ListOrdinaryMessages` now has a PostgreSQL-backed roomserver history-visibility fixture for the `RSAPI != nil` path.
   - Completed tenth Phase D slice: channel-content backfill now has PostgreSQL reopen coverage proving backfilled post/comment/reaction projections and counts survive service/store reload.
   - Completed eleventh Phase D slice: `reports.submit` now has PostgreSQL-backed coverage proving the Matrix system notice, stored report row, and system conversation survive service/store reload.
   - Completed twelfth Phase D slice: member role and mute projections now have PostgreSQL-backed reload coverage through `groups.members` and `channels.members`, proving owners stay unmuted owners and ordinary members stay muted members after service/store reload.
   - Completed thirteenth Phase D slice: channel member and pending-join counts now have PostgreSQL-backed reload coverage through `channels.list` and `channels.public.get`, proving stored `member_count` and `pending_join_count` survive service/store reload.
   - Completed fourteenth Phase D slice: contact deletion/reactivation reload coverage now proves deleted direct contacts remain hidden from `contacts.list`, retain their old room identity in raw storage after service/store reload, and can restore that original room through `contacts.request`.
   - Completed fifteenth Phase D slice: member count calculation now lives in neutral `internal/dirextalkprojection.ProductMemberCounts`, with p2p service refresh and conversation views calling that helper instead of keeping a local duplicate.
   - Completed sixteenth Phase D slice: product Matrix state-event content builders for direct/group/channel room profiles, member policy, and join requests now live in neutral `internal/dirextalkstate`; p2p keeps facade wrappers plus publish orchestration.
   - Completed seventeenth Phase D slice: product Matrix state parsing now lives in `internal/dirextalkstate`; p2p projectors consume parsed room-profile and join-request content instead of keeping local duplicate parsing.
   - Completed eighteenth Phase D slice: projection profile helpers for channel/group/member-policy/join-request records now live in `internal/dirextalkprojection`; `p2p/projection` keeps compatibility wrappers and roomserver event timestamp handling.
   - Completed nineteenth Phase D slice: shared conversation value records and helpers now live in `internal/dirextalkdomain`; `p2p/domain` keeps contact/group response adapters.
...
8. Phase E: split storage boundaries after behavior is stable.
   - Completed first Phase E slice: durable contact and group storage records now live in `internal/dirextalkdomain`; `p2p` keeps response-shaped contact/group facades and conversion helpers.
   - Completed second Phase E slice: durable channel post and channel comment storage records now live in `internal/dirextalkdomain`; `p2p` keeps response-only reaction/operation/conversation enrichment in the facade.
   - Completed third Phase E slice: `p2p/service.go:Store` now composes focused store interfaces, with aggregate and per-interface compile-time contract tests.
   - Phase E checkpoint: field ownership is classified above. Do not move storage writes into lower-level packages until the target table's source-of-truth/projection rule has DB-backed reload coverage and migration idempotency coverage.
