...
| Matrix history DTOs and filtering. | `internal/dirextalkmcp/types.go`: `MessageSummary`, `Page`, `MessagePageResult`, `Event`; `p2p/matrixhistory/types.go`: aliases/wrappers; `p2p/matrixhistory/reader.go`: `ListOrdinaryMessages`, `ListChannelContent`; `syncapi/agenthistory/reader.go`: `Reader.ListOrdinaryMessages`; `p2p/mcp_api.go`: `mcpMessagesList` | Shared DTOs/helpers now live in `internal/dirextalkmcp`; future work can move reader implementations without a `syncapi -> p2p` type dependency. | `syncapi` still imports `p2p/matrixhistory` for the reader package, but the shared DTO/helper owner is no longer a p2p package. |
...
| --- | --- | --- | --- |
| `syncapi` currently imports a `p2p` subpackage. | `syncapi/agenthistory/reader.go`: imports the repo's `p2p/matrixhistory` package; `p2p/matrixhistory/types.go`: aliases to `internal/dirextalkmcp`; `p2p/mcp_api.go`: `mcpMessagesList` | If p2p later imports `syncapi/agenthistory` directly, the graph can become `p2p -> syncapi -> p2p`. | Shared history DTOs/helpers are now neutral; later Phase D work should move or duplicate only the reader adapter package boundary before importing syncapi from p2p. |
