...
| Product-originated Matrix event/member/redaction execution. | `p2p/dendrite/dendrite_transport.go`: `CreateRoom`; `p2p/dendrite/dendrite_transport_send.go`: `SendMessage`, `SendStateEvent`; `p2p/dendrite/dendrite_transport_membership.go`: `InviteUser`, `JoinRoom`, `LeaveRoom`, `KickUser`; `p2p/dendrite/dendrite_transport_queries.go`: `UpdateMemberProfile`, `RedactEvent`; `internal/productpolicy/productpolicy.go`: `ValidateClientEvent`, `ValidateClientMembership`, `ValidateClientRedaction`; `p2p/transportapi/transport.go`: DTOs backed by neutral `internal/dirextalkdomain` aliases | Move Dendrite roomserver adaptation to a lower-level product Matrix adapter package now that transport DTO/domain dependencies are neutralized. Keep `internal/productpolicy` as write-rule owner. | p2p should orchestrate product workflows, not build and authorize roomserver events by hand. |
| Matrix history DTOs and filtering. | `internal/dirextalkmcp/types.go`: `MessageSummary`, `Page`, `MessagePageResult`, `Event`; `p2p/matrixhistory/types.go`: compatibility aliases/wrappers; `p2p/matrixhistory/reader.go`: `ListOrdinaryMessages`, `ListChannelContent`; `syncapi/agenthistory/reader.go`: `Reader.ListOrdinaryMessages`; `p2p/mcp_api.go`: `mcpMessagesList` | Shared DTOs/helpers now live in `internal/dirextalkmcp`; `syncapi/agenthistory` imports that neutral package directly. Future work can move reader implementations without a `syncapi -> p2p` type dependency. | Phase D removed the `syncapi -> p2p/matrixhistory` import edge; keep future reader moves compile-checked and add parity fixtures before changing behavior. |
...
| --- | --- | --- | --- |
| `syncapi` must not import a `p2p` subpackage. | `syncapi/agenthistory/reader.go`: imports `internal/dirextalkmcp` directly for `Page`, `MessageSummary`, `MessagePageResult`, and pagination helpers; `p2p/matrixhistory/types.go`: compatibility aliases to `internal/dirextalkmcp`; `p2p/mcp_api.go`: `mcpMessagesList` | Reintroducing a `syncapi -> p2p` import would make a future `p2p -> syncapi/agenthistory` adapter move cycle-prone. | Keep `syncapi/agenthistory` on neutral DTOs/helpers only; `rg "github.com/YingSuiAI/dirextalk-message-server/p2p" syncapi` should stay empty. |
| Transport DTOs must stay neutral after the Phase D split. | `p2p/transportapi/transport.go`: imports `internal/dirextalkdomain`; `internal/dirextalkdomain/types.go`: `Channel`, `MemberRecord`; `p2p/domain/types.go`: compatibility aliases; `p2p/dendrite/*`: imports `p2p/transportapi` | Reintroducing `p2p/domain` into `transportapi` would recreate a lower-level-to-facade dependency before moving `DendriteTransport`. | Keep transport-shared records in `internal/dirextalkdomain`; `rg "p2p/domain" p2p/transportapi` should stay empty. |
...
| Import-cycle safety is not represented by a small focused check. | `syncapi/agenthistory/reader.go`: neutral `internal/dirextalkmcp` import; `p2p/transportapi/transport.go`: neutral `internal/dirextalkdomain` import | `go test ./...` catches cycles late; add package-boundary review checks or keep Phase D moves small and compile after each package move. |
...
7. Phase D: move lower-level adapters in dependency-safe order.
   ...
   - Next Phase D slice: move Dendrite transport implementation from `p2p/dendrite/*` after productpolicy/write-path tests are kept passing.
