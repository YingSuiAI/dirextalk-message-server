...
Plugin management actions are owner-only. They install and manage official catalog plugins whose Docker images belong to the `dirextalk` Docker Hub organization; digest metadata is optional and used only when explicitly present. The message-server keeps durable plugin/job/config state and may run Docker operations when deployed with `docker-compose.plugins.yml` and `P2P_PLUGIN_DOCKER_ENABLED=true`. `plugins.enable` injects runtime values through a temporary Docker env-file. Agent receives `DIREXTALK_BASE_URL`, `DIREXTALK_AGENT_TOKEN`, display/system prompt defaults, tools, skills, and MCP server configuration. Ops receives `DIREXTALK_BASE_URL`, `OPS_BACKUP_ROOT`, `OPS_MAX_BACKUPS`, `OPS_MESSAGE_SERVER_CONTAINER`, and `OPS_POSTGRES_CONTAINER`; it is the only official plugin allowed to mount Docker socket and the Ops backup volume. Official Agent model provider, base URL, model IDs, model API keys, default selection, aliases, and inference parameters are client-local: the selected `model_profile` is sent per `plugins.invoke` or WS `client.plugin_stream` call and is not persisted, returned, or injected into container env by the message-server. `DIREXTALK_BASE_URL` is taken from `P2P_PLUGIN_BACKEND_BASE_URL` when set; this should point at the backend's stable Docker-network HTTP name, normally `http://message-server:8008`, `http://dendrite-a:8008` for the local dual/x1 service, or the public server URL such as `https://x1.dirextalk.ai` when a deployment intentionally routes plugin callbacks through the public endpoint. Standard MCP protocol serving, external MCP client connections, skills, model-provider configuration, context compression, and Agent orchestration live in the official Agent plugin; the backend keeps only the fixed `mcp.*` capability actions. Ops owns status, backup, migration export, and cleanup planning. Direct SQL purge of Matrix event history is not part of the first-version Ops contract.
