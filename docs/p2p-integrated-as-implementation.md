...
MCP actions are owner-scoped proxy operations. `agent_token` authorizes only the fixed MCP action set, and the handlers read/write product data from the portal owner view. Ordinary MCP history reads reuse the current owner `access_token` for Matrix Client-Server history and never create or refresh a Matrix session, so they cannot evict the owner's active clients.
