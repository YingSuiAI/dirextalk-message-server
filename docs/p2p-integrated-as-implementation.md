...
Plugin management actions are owner-only. `io.dirextalk.agent` is now a native message-server runtime and remains visible in `plugins.catalog.list` even when the Docker plugin runner is disabled. Ops and future non-Agent plugins still use official Docker images from the `dirextalk` Docker Hub organization; digest metadata is optional and used only when explicitly present. The message-server keeps durable plugin/job/config state and runs non-Agent Docker operations from the single-node `docker-compose.p2p.yml` deployment, which enables `P2P_PLUGIN_DOCKER_ENABLED=true`, mounts `/var/run/docker.sock`, and sets `P2P_PLUGIN_DOCKER_NETWORK=dirextalk-p2p_default`. Ops receives `DIREXTALK_BASE_URL`, `OPS_BACKUP_ROOT`, `OPS_MAX_BACKUPS`, `OPS_MESSAGE_SERVER_CONTAINER`, `OPS_POSTGRES_CONTAINER`, `OPS_POSTGRES_USER`, and `OPS_POSTGRES_PASSWORD`; it is the only official plugin allowed to mount Docker socket and the Ops backup volume. Native Agent model provider, base URL, model IDs, model API keys, default selection, aliases, and inference parameters are request-local: the selected `model_profile` is sent per `plugins.invoke` or WS `client.plugin_stream` call and is not persisted, returned, or injected into any container env by the message-server. Skills, third-party MCP client metadata, runtime CLI tools, server-side memory, context compression, and Agent orchestration live in the native Agent data dir configured by `P2P_NATIVE_AGENT_DATA_DIR`. Agent knowledge action names are reserved but first-version runtime reports unsupported and does not load vector index dependencies. The backend keeps the fixed `mcp.*` capability actions for owner/agent-token access. Ops owns status, backup, migration export, cleanup planning, backup progress polling, chunked backup download, and explicit confirmed restore from existing backup records. Direct SQL purge of Matrix event history is not part of the first-version Ops contract.
