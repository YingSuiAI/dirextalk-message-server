...
Plugin management actions are owner-only. They install and manage official catalog plugins whose Docker images belong to the `dirextalk` Docker Hub organization; digest metadata is optional and used only when explicitly present. When the Docker plugin runner is disabled, `plugins.catalog.list` returns an empty plugin list and clients should hide plugin entry points. The message-server keeps durable plugin/job/config state and runs Docker operations from the single-node `docker-compose.p2p.yml` deployment, which enables `P2P_PLUGIN_DOCKER_ENABLED=true`, mounts `/var/run/docker.sock`, and sets `P2P_PLUGIN_DOCKER_NETWORK=dirextalk-p2p_default`. `plugins.enable` injects runtime values through a temporary Docker env-file. Agent receives `DIREXTALK_BASE_URL`, `DIREXTALK_AGENT_TOKEN`, display/system prompt defaults, tools, skills, and MCP server configuration. Ops receives `DIREXTALK_BASE_URL`, `OPS_BACKUP_ROOT`, `OPS_MAX_BACKUPS`, `OPS_MESSAGE_SERVER_CONTAINER`, `OPS_POSTGRES_CONTAINER`, `OPS_POSTGRES_USER`, and `OPS_POSTGRES_PASSWORD`; it is the only official plugin allowed to mount Docker socket and the Ops backup volume. Official Agent model provider, base URL, model IDs, model API keys, default selection, aliases, and inference parameters are client-local: the selected `model_profile` is sent per `plugins.invoke` or WS `client.plugin_stream` call and is not persisted, returned, or injected into container env by the message-server. `DIREXTALK_BASE_URL` is taken from `P2P_PLUGIN_BACKEND_BASE_URL` when set; plugin deployment is single-node only and should point at the stable backend Docker-network HTTP name, normally `http://message-server:8008`, or the public server URL such as `https://x1.dirextalk.ai` when a deployment intentionally routes plugin callbacks through the public endpoint. Standard MCP protocol serving, external MCP client connections, skills, model-provider configuration, context compression, and Agent orchestration live in the official Agent plugin; Agent knowledge action names are reserved but first-version runtime reports unsupported and does not load vector index dependencies. The backend keeps only the fixed `mcp.*` capability actions. Ops owns status, backup, migration export, cleanup planning, backup progress polling, chunked backup download, and explicit confirmed restore from existing backup records. Direct SQL purge of Matrix event history is not part of the first-version Ops contract.
