...
- Native Agent supports `openai`, `anthropic`, `deepseek`, `gemini`, `xai`, `openai_compatible`, and `openrouter`. Gemini uses Google's OpenAI-compatible endpoint `https://generativelanguage.googleapis.com/v1beta/openai`; xAI uses its OpenAI-compatible endpoint `https://api.x.ai/v1`. Both use request-scoped provider API keys.
