...
- Native Agent supports `openai`, `anthropic`, `deepseek`, `gemini`, `xai`, `openai_compatible`, and `openrouter`. Client-visible provider bases include their version roots: Anthropic `https://api.anthropic.com/v1`, DeepSeek `https://api.deepseek.com/v1`, Gemini `https://generativelanguage.googleapis.com/v1beta`, and xAI `https://api.x.ai/v1`. The server derives Gemini's OpenAI-compatible chat subpath internally and uses Gemini's native `/models` route. Provider API keys remain request-scoped.
...
- `agent.models.list` uses request-scoped provider/base_url/api_key to fetch real provider model lists and rejects a provider fetch without an explicit `base_url`. Anthropic calls its native `GET /v1/models` endpoint with `x-api-key` and `anthropic-version`; Gemini and xAI call their OpenAI-compatible `GET /models` endpoints with bearer authentication. It must not persist API keys, maintain server-side model profiles for the current client, or invent model context/temperature/top_p/max-output/reasoning defaults.
