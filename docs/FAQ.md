...
## Why does Dirextalk Message Server exist?

Dirextalk Message Server aims to provide a matrix compatible server that has low resource usage compared to [Synapse](https://github.com/matrix-org/synapse).
It also aims to provide more flexibility when scaling either up or down.
Dirextalk Message Server's code is also very easy to hack on which makes it suitable for experimenting with new matrix features such as peer-to-peer.

## Is Dirextalk Message Server stable?

Mostly, although there are still bugs and missing features. If you are a confident power user and you are happy to spend some time debugging things when they go wrong, then please try out Dirextalk Message Server. If you are a community, organisation or business that demands stability and uptime, then Dirextalk Message Server is not for you yet - please install Synapse instead.

## Is Dirextalk Message Server feature-complete?

No, although a good portion of the Matrix specification has been implemented. Mostly missing are client features - see the [readme](https://github.com/YingSuiAI/dirextalk-message-server/blob/main/README.md) at the root of the repository for more information.

## Why doesn't Dirextalk Message Server have "x" yet?

Dirextalk Message Server development is currently supported by a small team of developers and due to those limited resources, the majority of the effort is focused on getting Dirextalk Message Server to be
...
## Is there a migration path from Synapse to Dirextalk Message Server?

No, not at present. There will be in the future when Dirextalk Message Server reaches version 1.0. For now it is not
possible to migrate an existing Synapse deployment to Dirextalk Message Server.

## Can I use Dirextalk Message Server with an existing Synapse database?

No, Dirextalk Message Server has a very different database schema to Synapse and the two are not interchangeable.

## Can I configure which port Dirextalk Message Server listens on?

Yes, use the cli flag `-http-bind-address`.

## I've installed Dirextalk Message Server but federation isn't working
...
## Does Dirextalk Message Server work with my favourite client?
...
## Is there a public instance of Dirextalk Message Server I can try out?

Use [dendrite.matrix.org](https://dendrite.matrix.org) which we officially support.

## Does Dirextalk Message Server support Space Summaries?

Yes

## Does Dirextalk Message Server support Threads?
...
## Does Dirextalk Message Server support push notifications?

Yes, we have experimental support for push notifications. Configure them in the usual way in your Matrix client.

## Does Dirextalk Message Server support application services/bridges?

Possibly - Dirextalk Message Server does have some application service support but it is not well tested. Please let us know by raising a GitHub issue if you try it and run into problems.
...
Yes, you can do this by disabling federation - set `disable_federation` to `true` in the `global` section of the Dirextalk Message Server configuration file.
...
## What data needs to be kept if transferring/backing up Dirextalk Message Server?
...
## Is there an upgrade guide for Dirextalk Message Server?
...
## Dirextalk Message Server is using a lot of CPU
...
## Dirextalk Message Server is using a lot of RAM

As above with CPU usage, some memory spikes are expected if Dirextalk Message Server is doing particularly heavy work
...
## Dirextalk Message Server is running out of PostgreSQL database connections

You may need to revisit the connection limit of your PostgreSQL server and/or make changes to the `max_connections` lines in your Dirextalk Message Server configuration. Be aware that each Dirextalk Message Server component opens its own database connections and has its own connection limit, even in monolith mode!

## VOIP and Video Calls don't appear to work on Dirextalk Message Server
...
to the endpoint URL configured in your Dirextalk Message Server configuration file only. The following is an
