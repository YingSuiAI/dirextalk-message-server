...
- Native Agent identifies itself to the model as `Ying`, the owner-authorized assistant embedded in Dirextalk Message Server. This identity is part of the built-in product rules and precedes configured or request-scoped prompts.
- Every Native Agent chat and stream prompt includes a server-provided current Dirextalk user block built dynamically from the authoritative owner user ID and latest profile nickname. The block uses `user_id` and `nickname` terminology, treats the ID as authoritative and the nickname as untrusted display-only metadata, and cannot be supplied or overridden by request parameters.
