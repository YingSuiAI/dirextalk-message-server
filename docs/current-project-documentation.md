...
Plugin management is owner-only and stays on the same body-action surface. `plugins.catalog.list`、`plugins.installed.list`、`plugins.install`、`plugins.enable`、`plugins.disable`、`plugins.uninstall`、`plugins.config.get`、`plugins.config.update`、`plugins.job.get`、`plugins.health`、`plugins.logs.tail`、`plugins.invoke`、`plugins.invoke.stream` 都需要 owner `access_token`；`agent_token` 不能调用这些 action。`io.dirextalk.agent` 是 message-server 内嵌 native runtime，Docker plugin runner 未启用时仍会出现在 `plugins.catalog.list`；Ops 和后续非 Agent Docker 插件只有在 runner 启用时展示和运行。非 Agent Docker 插件仍必须使用官方 catalog 中属于 `dirextalk` Docker Hub 组织的镜像；digest 是可选审计/回滚信息，不作为安装必要条件。Native Agent 的模型平台、密钥、别名、默认模型和推理参数是请求级配置；客户端在 `plugins.invoke` 或 WS `client.plugin_stream` 的 `params.model_profile` 中随本次调用传入选中的 `provider`、`model`、`base_url`、`api_key`、`temperature`、`top_p`、`top_k`、`max_output_tokens`、`context_window`、`reasoning_mode` 等字段。`agent.models.list` 返回的 `context_length`、`max_output_tokens` 等 metadata 只用于客户端本地生成模型默认参数；`agent.runtime.inspect` 可检查 native data dir、skills、MCP server 和 CLI runtime 状态，但不回显 API key。message-server 不持久化、不回显、不注入模型平台、API 地址、模型 ID、参数或密钥。启用 native Agent 时不会创建 Agent 容器，也不会注入 env/volumes；skills、MCP metadata、runtime CLI 和 memory 存在 `P2P_NATIVE_AGENT_DATA_DIR` 下。普通插件调用用 HTTP `plugins.invoke`；流式 Agent 调用必须通过 owner WS `client.plugin_stream`，HTTP `plugins.invoke.stream` 只返回 `action requires websocket`。Agent 对话上下文可由客户端传入，也可由 native runtime 按 `conversation_id` 保留 recent memory 并通过 `agent.context.compress` 压缩。Agent 知识库 action 名称保留但第一版不支持，相关 action 只返回 unsupported 或拒绝写入，客户端不展示知识库入口且不发送 `knowledge_enabled`。
...
- MCP action 是 owner-scoped 代理能力：`agent_token` 只负责授权固定 MCP action，联系人列表/搜索、房间搜索、成员身份列表、普通消息默认发送/读取、频道帖子/评论读取和评论创建都按 portal owner 视角操作；普通 `mcp.messages.send` 不能发送到配置的 `agent_room_id`，agent room 回复只能由 gateway 使用 `agent_gateway`/`gateway_source` 标记路径以 `@agent:<server>` 发出；`mcp.messages.list` 复用当前 owner `access_token` 读取 Matrix history，不创建 `DIREXTALK_MATRIX_HISTORY` 设备，也不刷新 Matrix session，因此不会导致 owner 手机/浏览器 session 被踢下线；`mcp.contacts.list/search` 只返回 accepted 且未删除的 direct contacts，`mcp.messages.list` 返回 `created_at`、`sender_mxid`、`sender_display_name`、`sender_domain` 和 `sender_localpart`，`mcp.room_members.list` 只允许查询已知 Dirextalk 产品房间/会话，返回 Matrix 成员身份、角色、头像和 profile fallback 后的展示名，`joined_at` 使用 RFC3339 字符串。MCP 读接口使用 `from_time`/`to_time`、`cursor` 和 `limit` 按 newest-first 稳定分页，拒绝旧的 `from_ts`/`to_ts`；cursor 固定首次查询快照，新插入内容只会出现在新的无 cursor 查询中。`mcp.channel_posts.list` 返回帖子 `comment_count`、`like_count`、当前 owner 本地 `favorite_count` 和 `favorited_by_me`；频道普通聊天仍用 `mcp.messages.list` 读取。`agent.config.get/update` 返回和持久化 `avatar_url` 与 `mcp_blocked_room_ids`；黑名单房间不会出现在 `mcp.rooms.search`，其它直接定位黑名单房间或其频道帖子的 MCP 读写会返回 403。Native Agent 在 message-server 内负责标准 MCP client、skills、模型平台配置和 Agent orchestration；后端仍保留固定 `mcp.*` capability actions 和 owner-only `plugins.*` 管理/调用边界，不内置公开 `POST /_p2p/mcp`。
- Native Agent 对话是 server-backed 插件调用业务，独立于旧 connect/Codex bridge room 会话。客户端通过 `client.plugin_stream` 发送 `plugin_id`、`action` 和 `params`，服务端会把 `agent.chat` 自动映射到 native runtime 的 `agent.chat.stream`，并以 `server.plugin_stream.event` 持续返回 `delta`/`done`；客户端取消时发送 `client.plugin_stream.cancel`。Native Agent 还提供 `agent.models.list`、`agent.runtime.inspect`、`agent.runtime.install`、`agent.runtime.which`、`agent.runtime.run`、`agent.skills.registry.search`、`agent.skills.install`、`agent.skills.enable`、`agent.skills.disable`、`agent.skills.uninstall`、`agent.mcp.servers.list`、`agent.mcp.servers.install`、`agent.mcp.servers.enable`、`agent.mcp.servers.disable`、`agent.mcp.servers.uninstall`、`agent.mcp.registry.search` 和 `agent.config.propose_patch`；Agent 知识库 action 名称保留但第一版返回 unsupported，不加载向量索引依赖。Runtime CLI、skills、MCP server metadata 和 memory 均落在 `P2P_NATIVE_AGENT_DATA_DIR`。
