...
- 官方 Agent 插件对话是新的 server-backed 插件调用业务，独立于旧 connect/Codex bridge room 会话。客户端通过 `client.plugin_stream` 发送 `plugin_id`、`action` 和 `params`，服务端会把 `agent.chat` 自动映射到 runner 的 `agent.chat.stream`，并以 `server.plugin_stream.event` 持续返回 `delta`/`done`；客户端取消时发送 `client.plugin_stream.cancel`。
