...
Plugin management is owner-only and stays on the same body-action surface. `plugins.catalog.list`、`plugins.installed.list`、`plugins.install`、`plugins.enable`、`plugins.disable`、`plugins.uninstall`、`plugins.config.get`、`plugins.config.update`、`plugins.job.get`、`plugins.health`、`plugins.logs.tail`、`plugins.invoke`、`plugins.invoke.stream` 都需要 owner `access_token`；`agent_token` 不能调用这些 action。第一版只支持官方 catalog 中属于 `dirextalk` Docker Hub 组织的插件镜像；digest 是可选审计/回滚信息，不作为安装必要条件。官方 Agent 插件的模型平台、密钥、别名、默认模型和推理参数是客户端本地配置；客户端在 `plugins.invoke` 或 WS `client.plugin_stream` 的 `params.model_profile` 中随本次调用传入选中的 `provider`、`model`、`base_url`、`api_key`、`temperature`、`top_p`、`top_k`、`max_output_tokens`、`context_window`、`reasoning_mode` 等字段。message-server 不持久化、不回显、不注入模型平台、API 地址、模型 ID、参数或密钥。启用官方 Agent 插件时，message-server 只给 Agent 注入 `DIREXTALK_BASE_URL`、`DIREXTALK_AGENT_TOKEN`、展示名/系统提示词、tools、skills 和 MCP server 配置；其他插件不会收到 Agent token。`DIREXTALK_BASE_URL` 优先取 `P2P_PLUGIN_BACKEND_BASE_URL`，应配置为稳定的后端容器内网名（例如普通单机 compose 的 `http://message-server:8008`，dual/x1 本机部署的 `http://dendrite-a:8008`）或明确选择的真实公网域名（如 `https://x1.dirextalk.ai`），不能依赖测试脚本专用容器名。普通插件调用用 HTTP `plugins.invoke`；流式 Agent 调用必须通过 owner WS `client.plugin_stream`，HTTP `plugins.invoke.stream` 只返回 `action requires websocket`。Agent 对话上下文由客户端本地保存历史、自动/手动压缩摘要，并通过 `conversation_context` 随请求传给插件；压缩由官方插件动作 `agent.context.compress` 完成。

当前 x1 本机部署固定使用单节点 `docker-compose.p2p.yml` 的 `message-server` 容器，插件后端内网地址为 `http://message-server:8008`。`dendrite-a`、`dendrite-b`、`dendrite-c` 只属于三节点回归环境，不作为 x1 实际服务入口。

官方 Ops 插件 `io.dirextalk.ops` 面向单机私有部署运维，动作包括 `ops.status.get`、`ops.containers.list`、`ops.logs.tail`、`ops.backups.list`、`ops.backup.create`、`ops.backup.download_chunk`、`ops.backup.delete`、`ops.cleanup.plan`、`ops.cleanup.run`、`ops.rooms.cleanup.plan`、`ops.rooms.cleanup.run`、`ops.media.orphans.plan`、`ops.migration.export`、`ops.restore.plan`。Ops 是唯一允许由 Docker runner 挂载 Docker socket 和专用备份 volume 的官方插件；启用时注入 `OPS_BACKUP_ROOT`、`OPS_MAX_BACKUPS`、`OPS_MESSAGE_SERVER_CONTAINER`、`OPS_POSTGRES_CONTAINER`。第一版清理必须先 plan 后 confirm：聊天记录清理只做本地缓存、隐藏/归档计划和受控安全操作，不允许 Ops 插件直接 SQL 删除 Matrix 事件表；媒体清理默认只清缓存或明确孤儿文件，仍被消息/频道引用的媒体不删除。
