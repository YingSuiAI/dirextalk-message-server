...
Plugin management is owner-only and stays on the same body-action surface. `plugins.catalog.list`、`plugins.installed.list`、`plugins.install`、`plugins.enable`、`plugins.disable`、`plugins.uninstall`、`plugins.config.get`、`plugins.config.update`、`plugins.job.get`、`plugins.health`、`plugins.logs.tail`、`plugins.invoke`、`plugins.invoke.stream` 都需要 owner `access_token`；`agent_token` 不能调用这些 action。第一版只支持官方 catalog 中属于 `dirextalk` Docker Hub 组织的插件镜像；digest 是可选审计/回滚信息，不作为安装必要条件。`plugins.install`/`plugins.config.update` 可以在 `params.secrets.api_key` 或 Agent config 的 `api_key`/`model_profiles[].api_key` 中接收一次性明文密钥；message-server 会把它们转为 `secret:*` 引用并持久化到插件 secret store，`plugins.config.get` 只返回 `secret_status`，不回显密钥。启用官方 Agent 插件时，message-server 会把 `DIREXTALK_BASE_URL`、`DIREXTALK_AGENT_TOKEN`、模型 provider/model、`AGENT_API_KEY_REF`、模型 profiles、skills、MCP server 配置和 `secret:`/`env:` 解析后的环境变量通过临时 Docker env-file 注入插件容器；真实 API key 不写入插件持久化 config。`DIREXTALK_BASE_URL` 优先取 `P2P_PLUGIN_BACKEND_BASE_URL`，应配置为稳定的后端容器内网名（默认 `http://message-server:8008`）或明确选择的真实公网域名（如 `https://x1.dirextalk.ai`），不能依赖测试脚本专用容器名。普通插件调用用 HTTP `plugins.invoke`；流式 Agent 调用必须通过 owner WS `client.plugin_stream`，HTTP `plugins.invoke.stream` 只返回 `action requires websocket`。
