...
- 新建 group room 和 chat/text channel room 会在创建时写 `m.room.history_visibility=joined`，新成员只从自己的 Matrix join 之后接收普通 timeline 消息。`channel_type=post` 的帖子频道是单独规则：创建帖子频道、将已有 room 绑定为帖子频道、或把频道更新为 `post` 时显式写 `m.room.history_visibility=shared`，成员需要能看到当前频道已有的所有帖子和评论。当前规则不回溯修改已有房间。
