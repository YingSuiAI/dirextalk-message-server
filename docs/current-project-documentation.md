...
Protected action 通过 HTTP route 调用时需要 `Authorization: Bearer <access_token>`。登录后的客户端 product action 在 WS 已收到 `server.ready` 时优先走 `GET /_p2p/ws` 上的 `client.request`/`server.response`；点击时 WS 未 ready 或已断线时，当前 action 立即用 `POST /_p2p/query` 或 `POST /_p2p/command` 作为 owner HTTP fallback，同时 realtime WS 在后台继续重连。已发出 WS request 后响应丢失时，只对可安全重复的 action 做 HTTP fallback。`client.version.report`、`release.v1.status` 支持 owner HTTP/WS；`release.v1.apply`、`release.v2.status`、`release.v2.apply` 与 `portal.account.delete` 是 owner `access_token` 保护的 HTTP-only 命令。`release.v2.status` 只返回本机真实 server version、当前 device 报告的 client version 和 updater readiness/watchdog/token-free active-job 状态，不向客户端暴露 discovery plan；`release.v2.apply` 只接受 canonical `target_version`、小写 UUID `idempotency_key` 与 `confirm=apply_release_change`，拒绝 image/digest/url/plan token/shell/Compose/service 和未知基础设施字段。它先要求 updater direct-release contract v2，并对 target/key 执行绝不创建 job 的原子 replay-only 查询；已存在的 active/terminal job 可直接恢复 ticket。只有 replay miss 才进入新任务流程：读取固定中台 `appId=1&channelId=server` 记录，复核目标与中台最低客户端版本，再把三个公开字段和本机可信 `client_version` 送到 updater。Updater 必须验证 checksum 绑定的 canonical release index、index 内目标 manifest/digest、完整正式资产集合、不可变目标 digest、当前源镜像 digest、明确升级边、schema 兼容性及 manifest 客户端最小/最大范围，任一不符即失败关闭。`release.v1.apply` 只接受 updater 生成的 `plan_token`、UUID `idempotency_key` 与 `confirm=apply_release_change`，不接受 image/digest/version/shell/Compose/service 参数。`realtime.ws_ticket.create` 只接受 owner `access_token` 创建 owner WS ticket。`agent_token` 只允许通过 product body-action 访问 `agent.matrix_session.create`，并可访问标准 `POST /mcp` MCP endpoint，不能通过 HTTP fallback 调用 owner product action。固定 `mcp.*` HTTP body action 已从 `/_p2p/query` 和 `/_p2p/command` 删除；外部 MCP客户端必须使用 `POST /mcp` JSON-RPC。标准 MCP endpoint 与 `agent.matrix_session.create` 不迁移到 WS `client.request`。`GET /_p2p/ws` 只接受短期单次 owner WS ticket，不直接接受 bearer token。当前 public action 是：
...
Native Agent is a first-class server feature behind owner `agent.*` actions. The existing local Runner remains available for compatibility, while Chat, StreamChat, and runtime model-profile selection can use the independent `dirextalk-agent` service. Clients call `agent.chat`、`agent.models.list`、`agent.runtime.inspect/install/which/run`、`agent.skills.*`、`agent.mcp.*`、`agent.context.compress`、`agent.config.propose_patch` and built-in Dirextalk tool actions directly through `/query` or `/command` with owner `access_token`; `agent_token` is limited to `agent.matrix_session.create` through the product body-action surface and `POST /mcp`.

Remote model-profile selection uses the owner-only, HTTP-only and no-store `agent.runtime.profile.get/update` actions. Get accepts no parameters; update accepts only canonical UUID `idempotency_key`, catalog `profile_id`, nonnegative `expected_revision`, and optional `temperature`, `top_p`, `max_output_tokens`. Both return exactly `{available, configured, revision, available_profile_ids, profile}`, where `profile` is `null` or the de-secreted `{profile_id, provider, model, base_url, temperature, top_p, max_output_tokens, context_window, reasoning_effort}` projection. The independent Agent deployment owns the immutable secret-free catalog; its service key requires `runtime.read`, `runtime.write`, and `runtime.chat`, and the selected profile's credential enters Agent only through an operator-provisioned read-only mounted secret file. Remote `agent.chat` rejects legacy request-scoped `model_profile_id` and `model_profile`. The first profile selection uses the explicit baseline of 48 context messages, 12 memory messages, and 24 steps; later updates preserve non-profile config. After an ambiguous update result, clients first reconcile through GET and the current revision instead of rebuilding and blindly replaying a stale mutation.

Request-scoped provider/base URL/API key behavior and `agent.models.list` remain local Runner compatibility only; they do not configure or discover the remote Agent catalog. For that local path, provider-returned optional metadata remains authoritative and unset tuning continues to use provider defaults. Native Agent chat may expose all configured model-callable tools, including write tools, skill/MCP management writes, external MCP tools, installed runtime CLI tools, and `runtime__shell`; clients must not send a `dangerous_tools_confirm` field. High-risk operations are called out by the built-in Native Agent prompt and response text rather than blocked by a request confirmation gate. Native Agent subprocesses use a reduced runtime environment instead of inheriting all message-server environment variables; MCP stdio servers only receive that reduced environment plus explicitly configured `env`. Skill URL installation only fetches HTTPS public hosts and rejects localhost/private/link-local targets; GitHub repo installs accept owner/repo shorthand and, with `repo_url` plus `name`/`id`, automatically try common skills monorepo paths such as `skills/<name>/SKILL.md` before root `SKILL.md`. Native Agent chat always prepends built-in Dirextalk product rules before configured or request-scoped system prompts so the model prefers first-class Native Agent tools for skills, MCP, runtime, and Dirextalk operations instead of shelling out for examples such as `npx skills add`. Streaming Native Agent chat uses owner realtime WS `client.native_agent_stream` and `client.native_agent_stream.cancel`, returning `server.native_agent_stream.event/error/cancelled`; OpenAI-compatible reasoning models may include explicit `reasoning_content` on `delta` and final `done` payloads, and clients may display only that provider-returned reasoning text. This Native stream is independent of the real Matrix `agent_room_id`; Online Agent bridge messages still use Matrix Client-Server APIs and are not mirrored through Native stream frames.

Local Runner config remains in the native portal Agent config JSON, including shared display/avatar settings, `mcp_blocked_room_ids`, skills, MCP server metadata, runtime tool metadata, and memory settings. `agent.config.update` is not the independent Agent profile store; any legacy sanitized model-profile fields are local compatibility only and must not include API keys. Startup performs an idempotent sanitized import from old hidden `io.dirextalk.agent` plugin config when present. Runtime CLI files, installed skill files, MCP working data, and local memory artifacts stay under `P2P_NATIVE_AGENT_DATA_DIR`. Agent knowledge action names remain reserved and return unsupported in the first version.

`cloud_dialogue_mode=true` is a request-scoped Native Agent capability reduction for Cloud planning: it exposes only `native_agent_cloud_deployment_plan` plus the de-secretsed read-only `native_agent_cloud_status`, forces no-memory operation, and excludes shell/CLI, MCP, Skill/MCP management, ordinary tools, installed Skill prompts, and configuration/request prompt injection. It can create a connection-bound research Goal and queued Job, then report Cloud projections only; it does not authorize AWS mutations, secret use, approvals, ingress, service operations, or destruction. The independent `p2p/cmd/cloud-orchestrator` worker and `Dockerfile.cloud-orchestrator` have no Matrix, Agent-model, AWS, Docker-socket, or migration capability; it reaches the separate `p2p/cmd/cloud-researcher` process only through a TLS 1.3 mTLS typed endpoint. The Researcher alone reads its mounted model-key file and uses a no-proxy, no-redirect model transport; neither it nor the Orchestrator is in the default compose stack. Its output is only an experimental proposal until official-source/artifact and account-specific provider checks exist. Cloud events are relayed through the Message Server events module with an event-ID dedupe key rather than being written directly to websocket state.
