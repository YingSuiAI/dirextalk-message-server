...
Plugin management is owner-only and stays on the same body-action surface. `plugins.catalog.list`、`plugins.installed.list`、`plugins.install`、`plugins.enable`、`plugins.disable`、`plugins.uninstall`、`plugins.config.get`、`plugins.config.update`、`plugins.job.get`、`plugins.health`、`plugins.logs.tail` 都需要 owner `access_token`；`agent_token` 不能调用这些 action。第一版只支持官方 catalog 中的插件镜像和固定 digest。启用官方 Agent 插件时，message-server 会把 `DIREXTALK_BASE_URL`、`DIREXTALK_AGENT_TOKEN`、模型 provider/model、`AGENT_API_KEY_REF` 和 `env:` 引用的宿主环境密钥通过临时 Docker env-file 注入插件容器；真实 API key 不写入插件持久化 config。
