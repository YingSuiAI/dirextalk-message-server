...
`cloud_dialogue_mode=true` is a request-scoped Native Agent capability reduction for Cloud planning: it only exposes `native_agent_cloud_deployment_plan`, forces no-memory operation, and excludes shell/CLI, MCP, Skill/MCP management, ordinary tools, installed Skill prompts, and configuration/request prompt injection. It can create a research Goal only; it does not authorize AWS mutations, secret use, approvals, ingress, service operations, or destruction.
