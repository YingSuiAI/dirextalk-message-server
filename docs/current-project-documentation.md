...
Plugin management is owner-only and stays on the same body-action surface. `plugins.catalog.list`、`plugins.installed.list`、`plugins.install`、`plugins.enable`、`plugins.disable`、`plugins.uninstall`、`plugins.config.get`、`plugins.config.update`、`plugins.job.get`、`plugins.health`、`plugins.logs.tail`、`plugins.invoke`、`plugins.invoke.stream` 都需要 owner `access_token`；`agent_token` 不能调用这些 action。Docker plugin runner 未启用时 `plugins.catalog.list` 返回空 `plugins`，客户端应隐藏插件管理入口；runner 启用后才展示官方 catalog。第一版只支持官方 catalog 中属于 `dirextalk` Docker Hub 组织的插件镜像；digest 是可选审计/回滚信息，不作为安装必要条件。官方 Agent 插件的模型平台、密钥、别名、默认模型和推理参数是客户端本地配置；客户端在 `plugins.invoke` 或 WS `client.plugin_stream` 的 `params.model_profile` 中随本次调用传入选中的 `provider`、`model`、`base_url`、`api_key`、`temperature`、`top_p`、`top_k`、`max_output_tokens`、`context_window`、`reasoning_mode` 等字段。`agent.models.list` 返回的 `context_length`、`max_output_tokens` 等 metadata 只用于客户端本地生成模型默认参数；`agent.runtime.inspect` 可检查本次请求解析后的模型参数和第三方 MCP server 运行状态，但不回显 API key。message-server 不持久化、不回显、不注入模型平台、API 地址、模型 ID、参数或密钥。启用官方 Agent 插件时，message-server 只给 Agent 注入 `DIREXTALK_BASE_URL`、`DIREXTALK_AGENT_TOKEN`、展示名/系统提示词、tools、skills 和 MCP server 配置；其他插件不会收到 Agent token。`DIREXTALK_BASE_URL` 优先取 `P2P_PLUGIN_BACKEND_BASE_URL`，插件部署只面向单节点 compose，应配置为稳定的后端容器内网名（例如 `http://message-server:8008`）或明确选择的真实公网域名（如 `https://x1.dirextalk.ai`），不能依赖多节点测试脚本专用容器名。普通插件调用用 HTTP `plugins.invoke`；流式 Agent 调用必须通过 owner WS `client.plugin_stream`，HTTP `plugins.invoke.stream` 只返回 `action requires websocket`。Agent 对话上下文由客户端本地保存历史、自动/手动压缩摘要，并通过 `conversation_context` 随请求给插件；压缩由官方插件动作 `agent.context.compress` 完成。Agent 知识库代码保留但第一版不支持，相关 action 只返回 unsupported 或拒绝写入，客户端不展示知识库入口且不发送 `knowledge_enabled`。
...
- Agent token 不再有动态权限表，只能访问 `agent.matrix_session.create` 和固定 `mcp.*` HTTP action，不能调用 `realtime.ws_ticket.create` 创建 WS ticket；其他 protected action 只认 owner `access_token`。本地 bridge 可以使用 `agent.matrix_session.create` 得到的 Matrix session 监听 agents room 并回写消息；若部署设置 `DIREXIO_PRODUCT_AGENT_URL`，message-server 也可以把 owner 在真实 `agent_room_id` 发送的普通文本异步转发到 product-agent `/v1/message-server/new-message`，并把官方 Agent 插件的已保存 config 作为 `agent_config` 一起发送，避免 roomserver 投影链路等待模型响应。同一个 bridge 还支持 owner 通过 `plugins.invoke` 调用官方 Agent 子动作 `agent.memory.list`、`agent.memory.save`、`agent.memory.delete`，由 message-server 代理到 product-agent `GET/POST/DELETE /v1/agent/memory`；客户端不直连 product-agent，也不把 product-agent 地址暴露给用户界面。
- MCP action 是 owner-scoped 代理能力：`agent_token` 只负责授权固定 MCP action，联系人列表/搜索、房间搜索、成员身份列表、普通消息默认发送/读取、频道帖子/评论读取和评论创建都按 portal owner 视角操作；普通 `mcp.messages.send` 不能发送到配置的 `agent_room_id`，agent room 回复只能由 gateway 使用 `agent_gateway`/`gateway_source` 标记路径以 `@agent:<server>` 发出；`mcp.messages.list` 复用当前 owner `access_token` 读取 Matrix history，不创建 `DIREXTALK_MATRIX_HISTORY` 设备，也不刷新 Matrix session，因此不会导致 owner 手机/浏览器 session 被踢下线；`mcp.contacts.list/search` 只返回 accepted 且未删除的 direct contacts，`mcp.messages.list` 返回 `sender_mxid`、`sender_display_name`、`sender_domain` 和 `sender_localpart`，`mcp.room_members.list` 只允许查询已知 Dirextalk 产品房间/会话，返回 Matrix 成员身份、角色、头像和 profile fallback 后的展示名。`agent.config.get/update` 返回和持久化 `avatar_url` 与 `mcp_blocked_room_ids`；黑名单房间不会出现在 `mcp.rooms.search`，其它直接定位黑名单房间或其频道帖子的 MCP 读写会返回 403。标准 MCP server、外部 MCP client、skills、模型平台配置和 Agent orchestration 由官方 Agent 插件负责；后端只保留固定 `mcp.*` capability actions 和 owner-only `plugins.*` 管理/调用边界，不内置 `POST /_p2p/mcp`。
- 官方 Agent 插件对话是新的 server-backed 插件调用业务，独立于旧 connect/Codex bridge room 会话。客户端通过 `client.plugin_stream` 发送 `plugin_id`、`action` 和 `params`，服务端会把 `agent.chat` 自动映射到 runner 的 `agent.chat.stream`，并以 `server.plugin_stream.event` 持续返回 `delta`/`done`；客户端取消时发送 `client.plugin_stream.cancel`。官方 Agent 插件还提供 `agent.models.list`、`agent.runtime.inspect`、`agent.runtime.install`、`agent.skills.registry.search`、`agent.skills.install`、`agent.skills.uninstall`、`agent.mcp.servers.list`、`agent.mcp.servers.install`、`agent.mcp.servers.uninstall`、`agent.mcp.registry.search` 和 `agent.config.propose_patch` 供客户端或 Agent 对话完成模型发现、运行时检查、skills.sh 安装、第三方 MCP registry 安装、CLI 工具安装和结构化配置写入；Agent 知识库 action 名称保留但第一版返回 unsupported，不加载向量索引依赖；Agent 容器内置 `npx`、`npm` 与 `uv/uvx` 以运行 registry metadata 中的 npm/Python MCP server，并可在用户私有部署主机上安装 `agent-reach` 等对话需要的 CLI 能力。
...
- Agent 预览和最终可恢复正文都通过 Matrix 消息/编辑回写；客户端展示 Matrix timeline 的聚合编辑结果，不消费 `server.agent_stream`。可选 product-agent bridge 的最终回复同样以本地 `@agent:<server>` 写回 Matrix，并带 `io.dirextalk.agent_gateway=true` 与 `io.dirextalk.gateway_source="product-agent"`，避免回复再次触发 agent。
