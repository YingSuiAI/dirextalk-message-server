...
- 不新增 URL-shaped 产品接口；当前明确例外是标准 MCP Streamable HTTP endpoint `POST /_p2p/mcp`。其它新增产品能力优先使用稳定 action 和 params schema。
