...
func GetEvent(
	...
	events, err = db.FilterLocalHiddenEvents(ctx, rawUserID, rawRoomID, events)
	if err != nil {
		logger.WithError(err).Error("GetEvent: db.FilterLocalHiddenEvents failed")
		return util.JSONResponse{
			Code: http.StatusInternalServerError,
			JSON: spec.InternalServerError{},
		}
	...
	events[0] = preferRoomserverRedactedEvent(ctx, rsAPI, events[0])
...
type eventByIDQueryAPI interface {
	QueryEventsByID(ctx context.Context, req *api.QueryEventsByIDRequest, res *api.QueryEventsByIDResponse) error
}

func preferRoomserverRedactedEvent(ctx context.Context, rsAPI eventByIDQueryAPI, event *rstypes.HeaderedEvent) *rstypes.HeaderedEvent {
	if event == nil || eventHasRedaction(event) {
		return event
	}
	var res api.QueryEventsByIDResponse
	err := rsAPI.QueryEventsByID(ctx, &api.QueryEventsByIDRequest{
	...
	roomserverEvent := res.Events[0]
	if eventHasRedaction(roomserverEvent) {
		return roomserverEvent
	}
	return event
}

func eventHasRedaction(event *rstypes.HeaderedEvent) bool {
	if event == nil || event.PDU == nil {
		return false
	}
	return gjson.GetBytes(event.Unsigned(), "redacted_because").Exists()
}
