...
//nolint:gocyclo // Matrix context assembly keeps pagination, lazy-load, state, and event visibility checks together.
func Context(
	...
	filteredEvents, err = snapshot.FilterLocalHiddenEvents(ctx, device.UserID, roomID, filteredEvents)
	if err != nil {
		logrus.WithError(err).Error("unable to apply local event hide filter")
		return util.JSONResponse{
			Code: http.StatusInternalServerError,
			JSON: spec.InternalServerError{},
		}
	...
	eventsBeforeFiltered, err = snapshot.FilterLocalHiddenEvents(ctx, device.UserID, roomID, eventsBeforeFiltered)
	if err != nil {
		logrus.WithError(err).Error("unable to apply local event hide filter")
		return util.JSONResponse{
			Code: http.StatusInternalServerError,
	...
	eventsAfterFiltered, err = snapshot.FilterLocalHiddenEvents(ctx, device.UserID, roomID, eventsAfterFiltered)
	if err != nil {
		logrus.WithError(err).Error("unable to apply local event hide filter")
		return util.JSONResponse{
			Code: http.StatusInternalServerError,
	...
	start, end, err := getStartEnd(ctx, snapshot, eventsBeforeFiltered, eventsAfterFiltered)
	if err == nil {
