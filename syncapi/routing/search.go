...
func Search(req *http.Request, device *api.Device, syncDB storage.Database, fts fulltext.Indexer, from *string, rsAPI roomserverAPI.SyncRoomserverAPI) util.JSONResponse {
	...
	evs, err := snapshot.Events(ctx, wantEvents)
	if err != nil {
	...
	evs, err = snapshot.FilterLocalHiddenEvents(ctx, device.UserID, "", evs)
	if err != nil {
		logrus.WithError(err).Error("failed to filter local hidden events")
		return util.JSONResponse{
			Code: http.StatusInternalServerError,
			JSON: spec.InternalServerError{},
		}
	...
	for _, event := range evs {
		...
		eventsBefore, err = snapshot.FilterLocalHiddenEvents(ctx, device.UserID, event.RoomID().String(), eventsBefore)
		if err != nil {
			logrus.WithError(err).Error("failed to filter local hidden context events")
			return util.JSONResponse{
				Code: http.StatusInternalServerError,
				JSON: spec.InternalServerError{},
			}
		}
		eventsAfter, err = snapshot.FilterLocalHiddenEvents(ctx, device.UserID, event.RoomID().String(), eventsAfter)
		if err != nil {
			logrus.WithError(err).Error("failed to filter local hidden context events")
			return util.JSONResponse{
				Code: http.StatusInternalServerError,
