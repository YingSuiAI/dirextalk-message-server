...
func (p *PDUStreamProvider) CompleteSync(
	...
	for _, roomID := range joinedRoomIDs {
		...
		events.Events, err = snapshot.FilterLocalHiddenStreamEvents(ctx, req.Device.UserID, roomID, events.Events)
		if err != nil {
			req.Log.WithError(err).Error("p.DB.FilterLocalHiddenStreamEvents failed")
			return from
		}
...
func (p *PDUStreamProvider) addRoomDeltaToResponse(
	...
	recentStreamEvents, err = snapshot.FilterLocalHiddenStreamEvents(ctx, device.UserID, delta.RoomID, recentStreamEvents)
	if err != nil {
		return r.From, fmt.Errorf("snapshot.FilterLocalHiddenStreamEvents: %w", err)
	}
