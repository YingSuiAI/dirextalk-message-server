...
func TestRoutersError(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, path.Join(PublicFederationPathPrefix, "doesnotexist"), nil)
	...
	rec = httptest.NewRecorder()
	req = httptest.NewRequest(http.MethodGet, path.Join(DendriteAdminPathPrefix, "test"), nil)
