...
func TestServiceReadinessContractCarriesNoSelectableProbeTarget(t *testing.T) {
	digest := func(value string) string { return "sha256:" + strings.Repeat(value, 64) }
	raw := []byte(`{"schema":"dirextalk.service-readiness-task-issue/v1","execution_id":"execution-ready-0001","deployment_id":"deployment-ready-0001","service_id":"service-ready-0001","task_id":"readiness-task-0001","probe_kind":"stack_witnessed_fixed_worker_probe_v1","recipe_execution_manifest_digest":"` + digest("a") + `","install_evidence_digest":"` + digest("b") + `","semantic_expectation_digest":"` + digest("c") + `"}`)
...
func TestServiceReadinessChallengeIsFreshOpaqueAndAbsentFromReceipts(t *testing.T) {
	...
	task := ServiceReadinessTaskV1{Schema: ServiceReadinessTaskSchema, TaskID: "readiness-task-0001", ExecutionID: "execution-ready-0001", DeploymentID: "deployment-ready-0001", ServiceID: "service-ready-0001", ProbeKind: ServiceReadinessProbeKind, RecipeExecutionManifestDigest: "sha256:" + strings.Repeat("a", 64), InstallEvidenceDigest: "sha256:" + strings.Repeat("b", 64), SemanticExpectationDigest: "sha256:" + strings.Repeat("c", 64), Attempt: 1}
