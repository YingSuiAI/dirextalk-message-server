//go:build dendrite_upgrade_tests
...
RUN ./generate-config --ci --db "user=postgres database=postgres host=/var/run/postgresql/" > message-server.yaml
...
RUN sed -i 's/max_open_conns:.*$/max_open_conns: 100/g' message-server.yaml
...
do \n\
 ...
 \n\
sed -i "s/server_name: localhost/server_name: ${SERVER_NAME}/g" message-server.yaml \n\
PARAMS="--tls-cert server.crt --tls-key server.key --config message-server.yaml" \n\
...
RUN go build ./cmd/generate-config
RUN go build ./cmd/create-account
RUN ./generate-config --ci > message-server.yaml
RUN ./generate-keys --private-key matrix_key.pem --tls-cert server.crt --tls-key server.key
...
RUN sed -i "s%connection_string:.file:%connection_string: file:\/var\/lib\/postgresql\/15\/main\/%g" message-server.yaml
