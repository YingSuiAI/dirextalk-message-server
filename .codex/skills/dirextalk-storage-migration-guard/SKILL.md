---
name: dirextalk-storage-migration-guard
description: Design and validate Dirextalk durable state changes involving SQL schema, migrations, indexes, storage interfaces, read models, database selection, restart recovery, PostgreSQL/SQLite behavior, Docker database config, or persisted product state.
---

# Dirextalk Storage Migration Guard
...
- Identify the database family: setup config, roomserver, syncapi, userapi, mediaapi, federationapi, relayapi, appservice, or Dirextalk product read model.
...
- Keep P2P/Dirextalk product read models as projections unless a domain rule explicitly makes the table source-of-truth state.
...
Finish by selecting the final check set with `dirextalk-targeted-verification`.
