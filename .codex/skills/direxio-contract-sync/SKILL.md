...
- Protected product actions require bearer `access_token`. `agent_token` is accepted only for fixed `mcp.*` HTTP actions and `realtime.ws_ticket.create`; `GET /_p2p/ws` authenticates only short-lived single-use tickets. MCP actions are HTTP body actions and must not be migrated into WS `client.request`.
...
- Owner-visible Agent online state comes from native Matrix room state in the real `agent_room_id`: event type `io.direxio.agent.status`, state key `@agent:<server>`, and content field `online`. The server drives this state from Agent config plus agent-token WS gateway lifetime: `online=true` only while config is enabled and at least one gateway WS session is connected; startup/repair without a gateway writes `online=false`. `sync.bootstrap` only returns `agent_room_id`; do not add `agent_online` back, do not emit `agent.presence`, and do not drive this state from Matrix `m.presence`.
- Direxio realtime sync uses `realtime.ws_ticket.create` plus `GET /_p2p/ws` as the primary non-MCP product-event, request/response, and session-state path. Clients send `client.lifecycle`, `client.focus`, and `client.ack`; lifecycle may include `state`, `hidden`, and `flags`, and focus may include `focused` and `flags`. The server timestamps session state with server time. Owner WS sessions send non-MCP product actions as `client.request`; `client.command` is only a compatibility alias to that response path. Agent-token WS sessions may send `client.agent_stream` fragments only for the real configured `agent_room_id`; owner WS sessions receive them as ephemeral `server.agent_stream` display frames. These fragments are not P2P outbox events or Matrix history, and final agent replies must still be Matrix `m.room.message` events from `@agent:<server>`. A connected foreground, non-hidden WS session suppresses push only for the same focused room. Global Matrix account data `io.direxio.push.context` remains a 60-second server-clock fallback only when no fresh WS session exists. Do not add additional lifecycle/focus body actions unless the product contract changes explicitly.
