---
name: dirextalk-contract-sync
description: Keep Dirextalk public contracts synchronized when changing HTTP routes, Matrix extension routes, body actions, params, response fields, auth/public/Agent-token behavior, Postman examples, API change records, docs, or project-local contract rules.
---

# Dirextalk Contract Sync
...
- Dirextalk product routes are `GET /_p2p/health`, `POST /_p2p/query`, `POST /_p2p/command`, `GET /_p2p/ws`, and `GET /.well-known/portal/owner.json`.
- Dirextalk action requests use `{ "action": "...", "params": { ... } }`.
- Protected product actions require bearer `access_token`. `portal.account.delete` is a protected owner HTTP-only command and must not be migrated into WS `client.request`. `agent_token` is accepted only for `agent.matrix_session.create` and fixed `mcp.*` HTTP actions; it cannot create realtime WS tickets. `GET /_p2p/ws` authenticates only short-lived single-use owner tickets created with the owner `access_token`. MCP actions and `agent.matrix_session.create` are HTTP body actions and must not be migrated into WS `client.request`.
...
- Ordinary send, history, unread, search, and redaction use Matrix Client-Server APIs. Local history hiding uses `POST /_matrix/client/v1/io.dirextalk/rooms/{roomID}/local_delete`.
...
Finish by selecting checks with `dirextalk-targeted-verification`.
