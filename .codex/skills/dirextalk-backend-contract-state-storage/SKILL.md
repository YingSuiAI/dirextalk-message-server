...
- Bind `client.version.report` to the owner portal device/session authenticated by that HTTP request or WS ticket. Reject stale sessions and persist through a narrow device-CAS update; a same-device full portal save must not overwrite the client build. Serialize `portal.password` token/generation mutation and persistence with report validation/CAS through `matrixSessionMu`, releasing it before Matrix-session refresh to avoid recursive locking.
