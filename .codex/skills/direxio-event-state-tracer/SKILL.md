...
- Agent online state is native Matrix room state in the real `agent_room_id`: event type `io.direxio.agent.status`, state key `@agent:<server>`, content field `online`. It is true only while Agent config is enabled and at least one agent-token WS gateway session is connected; startup/repair without a connected gateway must write false. Do not mirror it through `sync.bootstrap.agent_online`, `agent.presence` SSE, Matrix `m.presence`, or agent-token `/_p2p/events` stream lifetime.
