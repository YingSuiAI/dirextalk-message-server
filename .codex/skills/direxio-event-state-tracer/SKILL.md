...
- Agent online state is native Matrix room state in the real `agent_room_id`: event type `io.direxio.agent.status`, state key `@agent:<server>`, content field `online`. The running local Agent bridge writes true/false through its Matrix `@agent:<server>` session; the server may write false during startup/repair or when Agent config is disabled, but must not infer true from Agent config, `/sync`, or WS sessions. Do not mirror it through `sync.bootstrap.agent_online`, `agent.presence` SSE, Matrix `m.presence`, or agent-token event streams.
