...
- App foreground/background is not inferred from `/sync`, read receipts, or pusher registration. Direxio clients report lifecycle state through global Matrix account data `io.direxio.push.context`; foreground clients should refresh it every 30 seconds with `expires_at_ms` about 60 seconds in the future, and write `{"foreground":false}` on background. Fresh `foreground=true` suppresses userapi notification insertion and HTTP push gateway delivery, while missing, malformed, expired, or `foreground=false` state keeps normal background push behavior.
