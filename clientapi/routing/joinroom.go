...
func JoinRoomByIDOrAlias(
	...
	if policyRoomID := productPolicyRoomIDForJoin(req.Context(), rsAPI, roomIDOrAlias); policyRoomID != "" {
		if err = productpolicy.ValidateClientMembership(req.Context(), rsAPI, productpolicy.ClientMembershipRequest{
			RoomID:     policyRoomID,
			SenderMXID: device.UserID,
			TargetMXID: device.UserID,
...
func productPolicyRoomIDForJoin(ctx context.Context, rsAPI roomserverAPI.ClientRoomserverAPI, roomIDOrAlias string) string {
	...
	var aliasRes roomserverAPI.GetRoomIDForAliasResponse
