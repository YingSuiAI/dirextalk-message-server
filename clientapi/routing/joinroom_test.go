...
func TestJoinRoomByIDOrAlias(t *testing.T) {
	...
	test.WithAllDatabases(t, func(t *testing.T, dbType test.DBType) {
		...
		productChannelResp, ok := resp.JSON.(createRoomResponse)
		if !ok {
			t.Fatalf("response is not a createRoomResponse: %+v", resp)
		}
		...
		testCases := []struct {
			...
			{
				name:                   "product approval channel rejects direct join before approval",
				device:                 bobDev,
				roomID:                 productChannelResp.RoomID,
				wantProductPolicyError: true,
			},
			{
				name:                   "product approval channel rejects alias join before approval",
				device:                 bobDev,
				roomID:                 productChannelResp.RoomAlias,
				wantProductPolicyError: true,
			},
		}
		...
		for _, tc := range testCases {
			t.Run(tc.name, func(t *testing.T) {
				...
				if tc.wantProductPolicyError {
					if joinResp.Code != http.StatusForbidden {
						t.Fatalf("expected product policy forbidden response, got %+v", joinResp)
					}
					body, err := json.Marshal(joinResp.JSON)
					if err != nil {
						t.Fatal(err)
					}
					if !bytes.Contains(body, []byte("channel join requires approved join request")) {
						t.Fatalf("expected product policy error body, got %s", body)
					}
					return
				}
