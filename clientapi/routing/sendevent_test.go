...
func TestSendEventAppliesDirextalkProductPolicy(t *testing.T) {
	...
	req, err := http.NewRequest("POST", "https://domain", io.NopCloser(strings.NewReader(`{"msgtype":"m.text","body":"blocked","p2p_kind":"channel_comment"}`)))
	if err != nil {
		t.Fatalf("failed to make request: %s", err.Error())
	}
	cfg := &config.ClientAPI{}
	device := &uapi.Device{UserID: senderUserID}

	resp := SendEvent(req, device, roomIDStr, eventType, nil, nil, cfg, rsAPI, nil)

	if resp.Code != http.StatusForbidden {
		t.Fatalf("expected product policy to return 403, got %v with %#v", resp.Code, resp.JSON)
	}
