...
import (
	...
	"github.com/YingSuiAI/dirextalk-message-server/setup/base"
	...
	clientutil "github.com/YingSuiAI/dirextalk-message-server/clientapi/httputil"
...
func wellKnownClientBaseURL(req *http.Request, configured string) string {
	...
	if forwardedProto := firstForwardedValue(req.Header.Get("X-Forwarded-Proto")); forwardedProto != "" {
...
func isAutoWellKnownClientName(configured string) bool {
	configured = strings.TrimSpace(configured)
	if strings.EqualFold(configured, "auto") {
		return true
	}
	parsed, err := url.Parse(configured)
	return err == nil && strings.EqualFold(parsed.Host, "auto")
}
...
func Setup(
	...
	if cfg.Matrix.WellKnownClientName != "" {
		...
		wkMux.Handle("/client", httputil.MakeExternalAPI("wellknown", func(r *http.Request) util.JSONResponse {
			baseURL := wellKnownClientBaseURL(r, cfg.Matrix.WellKnownClientName)
			response := WellKnownClientResponse{
				Homeserver: WellKnownClientHomeserver{baseURL},
			}
			if cfg.Matrix.WellKnownSlidingSyncProxy != "" {
